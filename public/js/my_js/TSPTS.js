    ...
    $.ajax({
        ...
        success: function(JsonObject){
        	...
        	if(JsonObject['result'] == 1)
        	{
                ...
                _status = "pending"
                if( JsonObject['final_packing_save_state'] == null ){
                    dlabel_to_scan = [po_num, lot_number, lotapp_quantity, JsonObject['counter'], 0, unique_num]
                }else{
                    if( JsonObject['final_packing_save_state']['dlabel_ctr'] == JsonObject['counter'] ){
                        dlabel_to_scan = [po_num, lot_number, lotapp_quantity, JsonObject['counter'], 1, unique_num ]
                        _style = "style='background-color: #51FF51;'"
                        _status = "scanned"
                    }else{
                        dlabel_to_scan = [po_num, lot_number, lotapp_quantity, JsonObject['counter'], 0, unique_num ]
                ...
                // if( JsonObject['final_packing_save_state'] == null ){
                //     pmi_blue_packing_lbl_to_scan = [ po_num, lot_number, lotapp_quantity, JsonObject['ww'], 0 ]
                // }else{
                //     if( JsonObject['final_packing_save_state']['dlabel_ctr'] == JsonObject['ww'] ){
                //         pmi_blue_packing_lbl_to_scan = [ po_num, lot_number, lotapp_quantity, JsonObject['ww'], 1 ]
                //         _style = "style='background-color: #51FF51;'"
                //         _status = "scanned"
                //     }else{
                //         pmi_blue_packing_lbl_to_scan = [ po_num, lot_number, lotapp_quantity, JsonObject['ww'], 0 ]
                ...
                html_blue_packing +=     "<td style='padding: 5px; width: 15%;' id='tray_check_list_id_5'>" + _status + "</td>"
    ...
    $.ajax({
        ...
        success: function(JsonObject){
            ...
            else
            {
                ...
                if(JsonObject['error']['add_label_correctness'] === undefined)
                {
                    $('#add_label_correctness').removeClass('is-invalid');
                }
                else
                {
                    $('#add_label_correctness').addClass('is-invalid');
                }
...
function TSPTSSubmitFinalPackingInspection_QC_conf()
{
     toastr.options = {
      "closeButton": false,
      "debug": false,
    ...
    $.ajax({
        url: "tspts_submit_final_packing_inspection_qc_conf",
        method: "post",
        data: $('#formFinalPackingInspection').serialize() + '&packing_id=' + packing_id + '&box_no=' + box_no + '&po_no=' + po_no + '&qty=' + qty,
        dataType: "json",
        beforeSend: function(){
