 ...
 //            ->make(true);
    ...
    public function get_wbs_material_kitting_rev(Request $request){
        ...
        $material_kitting = MaterialIssuanceSubSystem::with([
                                        ...
                                        ->where('po_no', $request->po_number)
                                        ->first();
                                        // return $material_kitting; //orig_a_drawing
        ...
        if(isset($material_kitting->device_name)){
        ...
        if($material_kitting != null){
            // $doc_details_query = RapidActiveDocs::where('series', 'LIKE', '%' . $device_name_print . '%')
            //     ->get();
            ...
            $doc_orig_a_drawing_query = $doc_details_query->filter(
                        function($item) {
                            return (($item->doc_type=='A Drawing')||($item->doc_type=='AA Drawing')||($item->doc_type=='KL Drawing'));
                        })
                        // ->where('orig_doc', 1)
                        ->flatten(1);

                        return $doc_orig_a_drawing_query;
            ...
            // $doc_o_drawing_query = $doc_details_query->filter(
            //             function($item) {
            //                 return (($item->doc_type=='IG Drawing')||($item->doc_type=='OGM Drawing')||($item->doc_type=='Urgent Direction'));
            //             })
            //             ->flatten(1);
    ...
    public function get_first_lot_data_by_po_no(Request $request)
    {
        // return 'true';
        if( isset( $request->validate ) ){
            $device = Device::where('name', $request->device_name)->get();
            $ttl_lot_cnt = ceil((int)$request->po_qty / $device[0]->ship_boxing);
        ...
        // return $data;
        if( isset($data[$i]->device_name) ){
            $data->device_details = Device::where('name', $data[$i]->device_name)->get();
        }
    ...
    public function save_prod_runcard_details_rev(Request $request){ // FOR MANUAL AND AUTOMATIC RUNCARD
        // return '$request';
        date_default_timezone_set('Asia/Manila');
