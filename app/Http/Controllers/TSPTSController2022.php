...
class TSPTSController2022 extends Controller
{
    ...
    {
        $oqcvirs = ProductionRuncard::with(['prod_runcard_station_many_details','prod_runcard_accessory_info','tspts_oqcvir_info' => function($query){
    ...
    {
        $oqcvirs = ProductionRuncard::with(['prod_runcard_station_many_details','prod_runcard_accessory_info','oqc_inspection','tspts_oqcvir_info' => function($query){
            $query->orderBy('created_at','desc');
        },'tspts_oqcvir_info.inspector_info'])->where('id', $request->id)->where('status',4)->get();
    ...
    public function tspts_view_lotapp_details(Request $request)
    {
        ...
        if(count($lotapp_details) > 0)
        {
            ...
            return response()->json(['result' => 1, 'lotapp_details' => $lotapp_details, 'lotapp_quantity' => $lotapp_quantity, 'list_of_trays' => $data, 'list_of_trays_2' => $data, 'counter' => $counter, 'packing_list' => $packing_list, 'final_packing_save_state' => $final_packing_save_state, 'ww' => $oqcLotApp[0]->ww, 'device_name' => $device_name, 'device_name_full' => $lotapp_details[0]->device_name]);
        }
        else
        {
            return response()->json(['result' => 2]);
...
' ."\n". $packing_code->device_code
    ...
    public function load_supervisorvalidation_pts_table(Request $request)
    {
        ...
        $data = ProductionRuncard::with(['prod_runcard_station_many_details','prod_runcard_accessory_info','tspts_oqcvir_info' => function($query){
            $query->where('result', 1);
    ...
    public function load_finalpacking_pts_table(Request $request)
    {
        ...
        }, 'tspts_packingconfirmation_info', 'tspts_packinginspection_info', 'tspts_supervisorvalidation_info', 'tspts_finalpackinginspection_info','tspts_finalpackinginspection_info.inspector_info'])->where('po_no', $request->po_num)->where('status',4)->get();
    ...
    public function get_finalpacking_result_by_id(Request $request){
        ...
        $ins_result_by_id = ProductionRuncard::with(['tspts_finalpackinginspection_info','prod_runcard_station_many_details',
            'wbs_kitting',
        ...
        if(isset($ins_result_by_id[0]['wbs_kitting'])){
            ...
            if( strpos( $device_name_print, "Burn-in Others" ) !== false) {
                $device_name_print = trim($device_name_print,"(Burn-in Others)");
            }
        }
