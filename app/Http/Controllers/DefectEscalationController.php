...
class DefectEscalationController extends Controller
{
 ...
 //            ->make(true);
    ...
    public function view_de_stations(Request $request){
        date_default_timezone_set('Asia/Manila');
        // $has_emboss = $request->has_emboss;
        $prod_defect_escalation_no = "";
        $prod_runcard_status = "";
        if(isset($request->prod_runcard_status)){
            $prod_runcard_status = $request->prod_runcard_status;
        }
        if(isset($request->prod_defect_escalation_no)){
            $prod_defect_escalation_no = $request->prod_defect_escalation_no;
        }

        $prod_runcardstations = DEStations::with([
                ...
                'defect_escalation_station_machine_details',
                'defect_escalation_station_machine_details.machine_info',
                'defect_escalation_station_operator_details',
                'defect_escalation_station_operator_details.operator_info',
            ...
            ->where('defect_escalation_id', $request->prod_runcard_id_query )
            ->get();

        return DataTables::of($prod_runcardstations)
            ->setRowId('id')
            ->addColumn('raw_action', function($prod_runcardstation) use ($prod_runcard_status, $prod_defect_escalation_no){
                $disabled = '';
                $prod_runcard_id = $prod_runcardstation->production_runcard_id;

                if(count($prod_runcardstation->defect_escalation_station_operator_details) > 0){
                    $disabled = '';
                    // if($has_emboss == 1){
                        // $disabled = 'disabled';
                        ...
                        // else{
                            ...
                            if($prod_runcard_status == 7 || $prod_runcard_status == 3 || $prod_runcard_status == 4){
                                ...
                                //                                 },
                                //                             ])
                                //                             ->where('lot_batch_no', $prod_defect_escalation_no)
                                //                             ->get();
                                //     // return $oqc_lot_app;
                                ...
                                // return $prod_defect_escalation_no;
                                // return $oqc_lot_app;
        ...
        if($validator->passes()){
            ...
            if($device != null){
                ...
                if(count($material_process) > 0){
                    ...
                    $defect_escalation_no = substr($request->txt_po_number, 0, -5);
                    ...
                    if($user->count() > 0){
                        if( isset($request->txt_prod_runcard_id_query) ){
                            ...
                            try {
                                ...
                                // Generate Runcard No
                                $prod_runcards = ProductionRuncard::where('defect_escalation_no', 'LIKE', $defect_escalation_no . "%")->orderBy('id', 'desc')->limit(1)->get();

                                $final_defect_escalation_no = "";
                                $explode_defect_escalation_no = "";

                                if($prod_runcards->count() > 0){
                                    $explode_defect_escalation_no = explode('-', $prod_runcards[0]->defect_escalation_no);

                                    $explode_defect_escalation_no[1] = str_pad($explode_defect_escalation_no[1] + 1,3,"0", STR_PAD_LEFT);

                                    $final_defect_escalation_no = implode('-', $explode_defect_escalation_no);
                                }
                                else{
                                    $final_defect_escalation_no = substr($request->txt_po_number, 0, -5) . '-001';
                                }

                                // return $defect_escalation_no;
                                ...
                                $prod_runcard_id = ProductionRuncard::insertGetId([
                                    ...
                                    'status'=> 1,
                                    'defect_escalation_no' => $final_defect_escalation_no,
                                    ...
                                    'comp_under_defect_escalation_no'=> 0,
                        ...
                        ProductionRuncard::where('id',$request->txt_prod_runcard_id_query)
                            ->update(
                                [
                                    ...
                                    'comp_under_defect_escalation_no'=> $request->sel_comp_under_defect_escalation_no,
                                    'last_updated_by' => Auth::user()->id,
                                    'updated_at' => date('Y-m-d H:i:s')
                                ]
                            );
    ...
    public function get_defect_escalation_by_po(Request $request){
        date_default_timezone_set('Asia/Manila');

        $defect_escalations = DefectEscalation::with([
                            'defect_escalation_station_many_details' => function($query){
        ...
        return DataTables::of($defect_escalations)
            ->addColumn('raw_action', function($defect_escalation){

                $result='<button style="width:30px;" runcard-id="' . $defect_escalation->id . '" title="Open details" class="btnOpenRuncardDetails btn btn-info btn-sm py-0"><i class="fa fa-info-circle fa-sm"></i></button>';

                $result .= ' <button style="width:30px;" runcard-id="' . $defect_escalation->id . '" runcard-no="' . $defect_escalation->defect_escalatio_no . '" title="Print QR Code" class="btnPrintRuncardNo btn btn-success btn-sm py-0"><i class="fa fa-print fa-sm"></i></button>';
                return $result;
            })
            ->addColumn('raw_status', function($defect_escalation){
                $result = '<i title="Ongoing" class="far fa-clock text-secondary px-2"></i>';

                if($defect_escalation->status == 3){
                    $result = '<i title="Submitted to OQC Lot App" class="fas fa-check text-success px-2"></i>';
                }

                return $result;
            })
            ->addColumn('total_no_of_ng', function($defect_escalation){
                $total_no_of_ng = null;

                if(count($defect_escalation->defect_escalation_station_many_details) > 0){
                    $total_no_of_ng = 0;
                    for($index = 0; $index < count($defect_escalation->defect_escalation_station_many_details); $index++){
                        $total_no_of_ng += $defect_escalation->defect_escalation_station_many_details[$index]->qty_ng;   
                    }
                }

                return $total_no_of_ng;
            })
            ->addColumn('last_runcard_po_qty', function($defect_escalation){
                $col_defect_escalation = null;
                $qty_output = 0;
                    if(count($defect_escalation->defect_escalation_station_many_details) > 0){
                        $total_no_of_emboss_ng = 0;
                        
                        $col_defect_escalation = collect($defect_escalation->defect_escalation_station_many_details)->sortByDesc('step_num')->slice(0, 1)->flatten(1);
                        $qty_output = $col_defect_escalation->pluck('qty_output')[0];
                    }
                return $qty_output;
            })
    ...
    public function get_defect_escalation_by_id (Request $request){
        date_default_timezone_set('Asia/Manila');
        $defect_escalation = DefectEscalation::with([
                                'defect_escalation_material_list' => function($query){
                                    $query->where('status', 1);
                                },
        ...
        return response()->json(['prod_runcard' => $defect_escalation]);
    }
                    ...
                    if($validator->passes()){
                         ...
                         if($device != null){
                            ...
                            if(count($material_process) > 0){
                                ...
                                $defect_escalation_no = substr($request->po_number, 0, -5);
                                $prod_runcard_id = "";
                                ...
                                if($runcard_id == null){
                                    ...
                                    try {
                                        ...
                                        if($prod_runcards->count() > 0){
                                            ...
                                            $final_defect_escalation_no = substr($request->po_number, 0, -5) . '-001';
                                        }
    ...
    // public function scan_emboss_lot_no(Request $request){
    //     date_default_timezone_set('Asia/Manila');
    ...
    public function edit_de_station(Request $request){
        date_default_timezone_set('Asia/Manila');
        $return_title = '<i class="fa fa-check-circle text-success"></i> Saved';
        $return_body = 'Record has been saved.';
        $result = false;
        ...
        if(isset($txt_prod_runcard_station_id_query)){
            $validator = Validator::make($data, [
                ...
                'txt_edit_prod_runcard_operator' => ['required'],
                'txt_edit_prod_runcard_station_machine' => ['required'],
                    ...
                    if($user[0]->position == 4){
                        if( $request->txt_prod_runcard_station_id_query ){
                            ...
                            // if($validator->passes()){
                                DB::beginTransaction();
                                try {
                                    DEStations::where('id',$request->txt_prod_runcard_station_id_query)
                                        ->update(
                                            [
                                    ...
                                    if(isset($request->txt_edit_prod_runcard_operator)){
                                        for($index = 0; $index < count($request->txt_edit_prod_runcard_operator); $index++){
                                            DEStationMachines::insert([
                                                'defect_escalation_id' => $request->txt_prod_runcard_id_query,
                                                'defect_escalation_station_id' => $request->txt_prod_runcard_station_id_query,
                                                'operator_id' => $request->txt_edit_prod_runcard_operator[$index],
                                            ]);
                                        }
                                    }
                ...
                if($user->count() > 0){
                    if($user[0]->position == 4){
                        // if( $request->txt_prod_runcard_station_id_query ){
                            ...
                            // if($validator->passes()){
                                DB::beginTransaction();
                                $last_prod_runcard_station = DEStations::where('defect_escalation_id', $request->txt_prod_runcard_id_query)->orderBy('id', 'desc')->first();
                                ...
                                try {
                                    $prod_runcard_station_id = DEStations::insertGetId([
                                        'defect_escalation_id'=> $request->txt_prod_runcard_id_query,
                                    ...
                                    if(isset($request->txt_edit_prod_runcard_operator)){
                                        for($index = 0; $index < count($request->txt_edit_prod_runcard_operator); $index++){
                                            DEStationOperators::insert([
                                                'defect_escalation_id' => $request->txt_prod_runcard_id_query,
                                                'defect_escalation_station_id' => $prod_runcard_station_id,
                                                'operator_id' => $request->txt_edit_prod_runcard_operator[$index],
                                            ]);
                                        }
                                    }
    ...
    public function save_defect_escalation_details (Request $request){ // FOR MANUAL AND AUTOMATIC RUNCARD
        date_default_timezone_set('Asia/Manila');
        $return_title = '<i class="fa fa-check-circle text-success"></i> Saved';
        $return_body = 'Record has been saved.';
        $result = false;
        ...
        if($validator->passes()){
            ...
            if($user->count() > 0){
                if( isset($request->txt_prod_runcard_id_query) ){
                    ...
                    try {
                        $prod_runcard_id = $request->txt_prod_runcard_id_query;
                        DefectEscalation::where('id',$request->txt_prod_runcard_id_query)
                            ->update(
                                [   
                    ...
                    try {
                        ...
                        // Generate Runcard No
                        $prod_runcards = DefectEscalation::where('defect_escalation_no', 'LIKE', $defect_escalation_no . "%")->orderBy('id', 'desc')->limit(1)->get();
                        ...
                        $prod_runcard_id = DefectEscalation::insertGetId([
                            'po_no'=> $request->txt_po_number,
                            'status'=> 1,
                            'defect_escalation_no' => $final_defect_escalation_no,
                            'product_type' => $request->product_type,
    ...
    public function submit_de_to_oqc_lot_app(Request $request){ // FOR MANUAL AND AUTOMATIC RUNCARD
        date_default_timezone_set('Asia/Manila');
        $return_title = '<i class="fa fa-check-circle text-success"></i> Saved';
        $return_body = 'Record has been saved.';
        $result = false;
    ...
    public function defect_escalation_save_material(Request $request){ // FOR MANUAL AND AUTOMATIC RUNCARD
        date_default_timezone_set('Asia/Manila');
        $return_title = '<i class="fa fa-check-circle text-success"></i> Saved';
        $return_body = 'Record has been saved.';
        $result = false;
    ...
    public function view_materials_by_defect_escalation_id(Request $request){
        $materials = DEMaterials::where('defect_escalation_id', $request->runcard_id)->with(['material_info'])
        ->get();
    ...
    public function save_rework(Request $request){
        // return $request->defect_escalation_id;
        date_default_timezone_set('Asia/Manila');
        session_start();
        ...
        if($request->ajax()){
            if(Auth::check()){
                // Add ReferenceType
                if(!isset($request->rework_id)){
                    $data = $request->all();

                    $rules = [
                        'defect_escalation_id' => 'required',
                        'unit_no' => 'required',
                        'mode_of_defect_id' => 'required',
                        'location_of_ng' => 'required',
                        'ng_qty' => 'required',
                        'scrap' => 'required',
                        'for_rework' => 'required',
                        'for_verification' => 'required',
                        'prod' => 'required',
                        'engg' => 'required',
                        'qc' => 'required',
                        'runcard_no' => 'required',
                    ...
                    try {
                        if($validator->passes()){
                            if($request->for_rework == 0){
                                $request->rework_qty = 0;
                                $request->result_qty_ok = 0;
                                $request->result_qty_scrap = 0;
                                $request->rework_code = 0;
                                $request->terminal_gauge = 0;
                                $request->dummy_lo = 0;
                                $request->dummy_mo = 0;
                            }
                            ...
                            DERework::insert([
                                ...
                                'defect_escalation_id' => $request->defect_escalation_id,
                                'mode_of_defect_id' => $request->mode_of_defect_id,
                                'location_of_ng' => $request->location_of_ng,
                                'ng_qty' => $request->ng_qty,
                                'scrap' => $request->scrap,
                                'for_rework' => $request->for_rework,
                                'for_verification' => $request->for_verification,
                                'prod' => $request->prod,
                                'engg' => $request->engg,
                                'qc' => $request->qc,
                                'rework_qty' => $request->rework_qty,
                                'rework_code' => $request->rework_code,
                                'result_qty_ok' => $request->result_qty_ok,
                                'result_qty_scrap' => $request->result_qty_scrap,
                                'terminal_gauge' => $request->terminal_gauge,
                                'dummy_lo' => $request->dummy_lo,
                                'dummy_mo' => $request->dummy_mo,
                                'operator' => $request->operator,
                                'date' => $request->date,
                ...
                // Edit ReferenceType
                    ...
                    try {
                        if($validator->passes()){
                            ...
                            DERework::where('id', $request->rework_id)
                                ->where('status', 1)
                                ->update([
                                    'unit_no' => $request->unit_no,
                                    'mode_of_defect_id' => $request->mode_of_defect_id,
    ...
    public function save_rework_verification(Request $request){
        date_default_timezone_set('Asia/Manila');
        session_start();
                ...
                try {
                    if($validator->passes()){
                        if($request->verification_type == 1){
                            $user_info = User::where('employee_id', $request->employee_no)
                                ->where('position', 1)
                                ->where('status', 1)
                                ->first();

                            if($user_info != null){
                                DERework::whereIn('id', $request->rework_id)
                                ->update([
                                    'prod' => $request->verification_result,
                                    'prod_id' => $user_info->id,
                                    'last_updated_by' => Auth::user()->id,
                                    'updated_at' => date('Y-m-d H:i:s'),
                                ]);
                                return response()->json(['auth' => 1, 'result' => 1, 'error' => null]);
                                ...
                                return response()->json(['auth' => 1, 'result' => 2, 'error' => null]); // Invalid User
                            }
                        }
                        else if($request->verification_type == 2){
                            $user_info = User::where('employee_id', $request->employee_no)
                                ->where('position', 9)
                                ->where('status', 1)
                                ->first();
                            ...
                            if($user_info != null){
                                DERework::whereIn('id', $request->rework_id)
                                ->update([
                                    'engg' => $request->verification_result,
                                    'engg_id' => $user_info->id,
                                    'last_updated_by' => Auth::user()->id,
                                    'updated_at' => date('Y-m-d H:i:s'),
                                ]);
                                return response()->json(['auth' => 1, 'result' => 1, 'error' => null]);
                        ...
                        else if($request->verification_type == 3){
                            $user_info = User::where('employee_id', $request->employee_no)
                                ->where('position', 2)
                                ->where('status', 1)
                                ->first();
                            ...
                            if($user_info != null){
                                DERework::whereIn('id', $request->rework_id)
                                ->update([
                                    'qc' => $request->verification_result,
                                    'qc_id' => $user_info->id,
                                    'last_updated_by' => Auth::user()->id,
                                    'updated_at' => date('Y-m-d H:i:s'),
                                ]);
    ...
    public function view_reworks(Request $request){
        if($request->ajax()){
            $data = DERework::where('status', 1)
                        ->where('defect_escalation_id', $request->defect_escalation_id)
                        ->with([
                            'operator_info',
                            'mode_of_defect_info',
                            'prod_info',
            ...
            return DataTables::of($data)
                ...
                ->addColumn('raw_status', function($row){
                    ...
                    if($row->status == 1){
                        $result .= '<span class="badge badge-pill bg-success">YES</span>';
                    }
                    else if($row->status == 0){
                        $result .= '<span class="badge badge-pill bg-warning">NO</span>';
                    }

                    return $result;
                })
                ->addColumn('raw_scrap', function($row){
                    $result = "";
                ...
                ->addColumn('raw_for_rework', function($row){
                    $result = "";

                    if($row->for_rework == 1){
                        $result .= '<span class="badge badge-pill bg-success">YES</span>';
                    }
                    else if($row->for_rework == 0){
                        $result .= '<span class="badge badge-pill bg-warning">NO</span>';
                    }

                    return $result;
                })
                ->addColumn('raw_for_verification', function($row){
                    $result = "";

                    if($row->for_verification == 1){
                        $result .= '<span class="badge badge-pill bg-success">YES</span>';
                    }
                    else if($row->for_verification == 0){
                        $result .= '<span class="badge badge-pill bg-warning">NO</span>';
                    }

                    return $result;
                })
                ->addColumn('raw_prod', function($row){
                    $result = "";

                    if($row->prod == 1){
                        $result .= '<span class="badge badge-pill bg-success">OK</span>';
                    }
                    else if($row->prod == 0){
                        $result .= '<span class="badge badge-pill bg-danger">SCRAP</span>';
                    }
                    else if($row->prod == 2){
                        $result .= '<span class="badge badge-pill bg-warning">REWORK</span>';
                    }

                    if($row->prod_info != null){
                        $result .= '<br>(' . $row->prod_info->name . ')';
                    }

                    return $result;
                })
                ->addColumn('raw_engg', function($row){
                    $result = "";
                    ...
                    if($row->engg_info != null){
                        $result .= '<br>(' . $row->engg_info->name . ')';
                    }

                    return $result;
                })
                ->addColumn('raw_qc', function($row){
                    $result = "";
                ...
                ->addColumn('raw_action', function($row){
                    $result = '';
                    if($row->status == 1){
                        $result .= '<button type="button" class="btn btn-xs btn-primary table-btns btnEditRework" rework-id="' . $row->id . '"><i class="fa fa-edit" title="Edit"></i></button>';
                ...
                ->addColumn('raw_terminal_gauge', function($row){
                    $result = "";

                    if($row->terminal_gauge == 1){
                        $result .= '<span class="badge badge-pill bg-success">YES</span>';
                    }
                    else if($row->terminal_gauge == 0){
                        $result .= '<span class="badge badge-pill bg-warning">NO</span>';
                    }

                    return $result;
                })
                ->addColumn('raw_dummy_lo', function($row){
                    $result = "";

                    if($row->dummy_lo == 1){
                        $result .= '<span class="badge badge-pill bg-success">YES</span>';
                    }
                    else if($row->dummy_lo == 0){
                        $result .= '<span class="badge badge-pill bg-warning">NO</span>';
                    }

                    return $result;
                })
                ->addColumn('raw_dummy_mo', function($row){
                    $result = "";

                    if($row->dummy_mo == 1){
                        $result .= '<span class="badge badge-pill bg-success">YES</span>';
                    }
                    else if($row->dummy_mo == 0){
                        $result .= '<span class="badge badge-pill bg-warning">NO</span>';
                    }
                ...
                ->addColumn('raw_checkbox', function($row){
                    ...
                    $result .= '<center><input type="checkbox" value="' . $row->id . '" class="chkSelectRework"></center>';

                    return $result;
                })
                ->rawColumns(['raw_status', 'raw_action', 'raw_scrap', 'raw_for_rework', 'raw_for_verification', 'raw_prod', 'raw_engg', 'raw_qc', 'raw_terminal_gauge', 'raw_dummy_lo', 'raw_dummy_mo', 'raw_checkbox'])
                ->make(true);
        }
        else{
            abort(403);
    ...
    public function get_rework_by_id(Request $request){
        // date_default_timezone_set('Asia/Manila');
        session_start();
        if($request->ajax()){
            if(Auth::check()){
                ...
                if($validator->passes()){
                    $rework_info = DERework::where('id', $request->rework_id)->first();

                    return response()->json(['auth' => 1, 'rework_info' => $rework_info, 'result' => 1]);
                }
                else{
                    return response()->json(['auth' => 1, 'rework_info' => null, 'result' => 0]);  
                }
            }
            else{
                return response()->json(['auth' => 0, 'result' => 0, 'error' => null]);
    ...
    public function generate_defect_escalation_qrcode(Request $request){
        date_default_timezone_set('Asia/Manila');
        $data = DefectEscalation::where('id', $request->runcard_id)->first();
        ...
        if($data->count() > 0){
                ...
                $runcard_qrcode = QrCode::format('png')
                            ->size(200)->errorCorrection('H')
                            ->generate($data->defect_escalation_no);
    ...
    public function getListRuncardNo(Request $request)
    {
        // $prod_runcard_ids = ProductionRuncard::select('id')->where('po_no', $request->po_no)->get();
        // $ids = [];
        // for ($i=0; $i < count($prod_runcard_ids); $i++)
        //     $ids[] = $prod_runcard_ids[$i]->id;



        // return response()->json(['data' => ProductionRuncardStation::select('id', 'runcard_no', 'production_runcard_id')
        //     ->where('status', 1)
        //     ->where('qty_ng', '>', 0)
        //     ->whereIn('production_runcard_id', $ids)
        //     ->get()
        // ]);

        $reworks = DB::table('de_reworks')
            ->select('runcard_no')
            ->where('po_no', $request->po_no)
            ->get();

        $ids = [];
        for ($i=0; $i < count($reworks); $i++)
            $ids[] = $reworks[$i]->runcard_no;

        return response()->json(['data' => 
            DB::table('production_runcards as prod')
                ->select('card.id', 'card.runcard_no', 'prod.lot_no')
                ->join('production_runcard_stations as card', 'prod.id', 'card.production_runcard_id')
                ->where('card.status', 1)
                ->whereNotIn('card.id', $ids)
                ->where('card.qty_ng', '>', 0)
                ->where('prod.po_no', $request->po_no)
                ->get()
        ]);
    }

    public function getDetailsOfRuncard(Request $request)
    {
        return response()->json(['data' => 
            ProductionRuncardStation::select('id', 'qty_ng')
                ->where('id', $request->id)
                ->get()
        ]);
    }
