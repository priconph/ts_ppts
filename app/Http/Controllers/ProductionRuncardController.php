...
{
    //
    public function update_prodn_rucard_lot_no(Request $request){
        try {
            date_default_timezone_set('Asia/Manila');
            DB::beginTransaction();

            //return json_
            ProductionRuncard::where('id',$request->prodn_lot_no)
            ->update(
                [
                    'runcard_no'=> $request->production_runcard_id,
 ...
 //            ->make(true);
    ...
    public function fn_view_setup_stations(Request $request){
        ...
        $devices = Device::where('barcode',$request->device_code)->where('status',1)->get();
    ...
    public function view_prod_runcard_stations(Request $request){
        ...
        return DataTables::of($prod_runcardstations)
            ...
            ->addColumn('raw_action', function($prod_runcardstation) use ($prod_runcard_status, $prod_runcard_no){
                ...
                $runcard____ = ProductionRuncard::find($prod_runcardstation->production_runcard_id);

                if( in_array(Auth::user()->position, [1, 2]) || in_array(Auth::user()->user_level_id, [2]) ){
                    return $result;
                }else{
                    if( $runcard____->status > 1 )
                        return '---';
                }
    ...
    public function fn_insert_prod_runcard(Request $request){
        ...
        if($validator->passes()){
            ...
            $device = Device::where('barcode', $request->txt_device_code)->where('status',1)->first();
                    ...
                    if($user->count() > 0){
                        if( isset($request->txt_prod_runcard_id_query) ){
                            ...
                            try {
                                ...
                                $device_info = Device::where('name', $request->device_name)->where('status',1)->first();
                                ...
                                if($prod_runcards->count() > 0){
                                    ...
                                    $explode_runcard_no[count($explode_runcard_no)-1] = str_pad($explode_runcard_no[count($explode_runcard_no)-1] + 1,4,"0", STR_PAD_LEFT);

                                    $final_runcard_no = implode('-', $explode_runcard_no);
                                }
                                else{
                                    // $final_runcard_no = substr($request->txt_po_number, 0, -5) . '-0001';
    ...
    public function get_prod_runcard_by_po(Request $request){
        ...
        $prod_runcards = ProductionRuncard::
                        with([
                            'wbs_kitting',
                            // 'prod_runcard_station_many_details' => function($query){
                            //     $query->orderBy('step_num', 'desc');
        ...
        return DataTables::of($prod_runcards)
            ->addColumn('raw_action', function($prod_runcard){
                ...
                $result .=' <button class="btn btn-success btn-sm py-0 d-none"  style="width:30px;" id="btnEditProdnLotNo" runcard-id="' . $prod_runcard->id.' " lot-no="' . $prod_runcard->runcard_no . '" po-no="'.$prod_runcard->po_no.'" lot-qty="'.$col_prod.'" title="Edit Lot Number" '.$disabled.'><i class="fa fa-edit fa-sm"></i></button>';
    ...
    public function get_wbs_material_kitting(Request $request){ //working function
        date_default_timezone_set('Asia/Manila');
        $material_kitting = MaterialIssuanceSubSystem::with([
                                            'device_info',
                                            'documents_details',
        ...
        return response()->json(['material_kitting' => $material_kitting, 'doc_a_drawing_query' => $doc_a_drawing_query, 'doc_g_drawing_query' => $doc_g_drawing_query,'po_no_qr' => $po_no]);
    }
        ...
        if(count($col_parts_prep_kit_issuance) > 0){
            ...
            if(count($arr_insert_issuance_id) > 0){
                ...
                if($runcard_id == null){
                    ...
                    if($validator->passes()){
                         $device = Device::where('name', $request->device_name)->where('status',1)->first();
                                ...
                                if($runcard_id == null){
                                    ...
                                    try {
                                        ...
                                        if($prod_runcards->count() > 0){
                                            ...
                                            // $final_runcard_no = substr($request->po_number, 0, -5) . '-0001';
                                            $final_runcard_no = $runcard_no . '-0001';
                                        }
                            ...
                            if( $request->txt_prod_runcard_station_id_query ){
                                ...
                                // if($validator->passes()){
                                    ...
                                    try {
                                        ...
                                        $stt_id = ProductionRuncardStation::where('id',$request->txt_prod_runcard_station_id_query)->get();
                                        $prd_runcard = ProductionRuncard::where('id', $stt_id[0]->production_runcard_id)->orderBy('id','DESC')->get();
                                        // $dvc = Device::where('name', $prd_runcard[0]->device_name)->get();
                                        $dvc = Device::where('name', $prd_runcard[0]->device_name)->where('status',1)->get(); //11172023

                                        // return response()->json(['error_msg' => $dvc]);
                                        // $first_prd_runcard = ProductionRuncard::where('po_no', $prd_runcard[0]->po_no)->get();
                                        $ttl_po_qty = 0;
                                        $all_ProductionRuncard = ProductionRuncard::where('po_no', $prd_runcard[0]->po_no)->orderBy('id','DESC')->get();
                                        for ($i=0; $i < count($all_ProductionRuncard); $i++) {
                                            $prd_run_stt = ProductionRuncardStation::where('production_runcard_id', $all_ProductionRuncard[$i]->id)->where('id', '!=', $stt_id[0]->id)->where('status', 1)->orderBy('id','DESC')->get();
                                            for ($j=0; $j < count($prd_run_stt); $j++)
                                                $ttl_po_qty += $prd_run_stt[$j]->qty_output;
                                                // $ttl_po_qty += $prd_run_stt[$j]->qty_input;
                                        }
                                        if( ( $ttl_po_qty + (int)$request->qty_input ) > $all_ProductionRuncard[0]->po_qty ){
                                            return response()->json(['error_msg' => 'You exceeding the total PO quantity.']);
                                            // return 'You exceeding the total PO quantity.';
                                        }


                                        $ttl_input = 0;
                                        $prd_run_stt = ProductionRuncardStation::where('production_runcard_id', $stt_id[0]->production_runcard_id)->where('id', '!=', $stt_id[0]->id)->where('status', 1)->get();
                                        for ($i=0; $i < count($prd_run_stt); $i++)
                                            $ttl_input += $prd_run_stt[$i]->qty_output;
                                            // $ttl_input += $prd_run_stt[$i]->qty_input;


                                        // return response()->json(['error_msg' => '1 / ' . (int)$request->qty_input . ' - ' . $dvc[0]->boxing]);

                                        // if( ( $ttl_input + (int)$request->qty_input ) > $dvc[0]->boxing )
                                        if( ( $ttl_input + (int)$request->qty_input ) > $dvc[0]->ship_boxing )
                                            return response()->json(['error_msg' => 'You exceeding the total input per lot.']);
                                            // return 'You exceeding the total input per lot.';
                            ...
                            // if( $request->txt_prod_runcard_station_id_query ){
                                ...
                                // if($validator->passes()){
                                    ...
                                    try {
                                        ...
                                        // return $prd_runcard_device_name;
                                        $prd_runcard_device_name = CommonController::getInstance()->validate_device_name($prd_runcard_device_name);
                                        //TODO: MIGZ 04/09/24 Catch the error if the Matrix need to be change.
                                        $dvc = Device::where('name',$prd_runcard_device_name)->where('status',1)->get();
                                        if( count($dvc) == 0 ){
                                            DB::rollback();
                                            return response()->json([
                                                "result" => 0,
                                                "error_msg" => "Please check the series in matrix",
                                            ]);
                                        }
                                        // $first_prd_runcard = ProductionRuncard::where('po_no', $prd_runcard[0]->po_no)->get();
                                        $ttl_po_qty = 0;
                                        $all_ProductionRuncard = ProductionRuncard::where('po_no', $prd_runcard[0]->po_no)->orderBy('id','DESC')->get();
                                        for ($i=0; $i < count($all_ProductionRuncard); $i++) {
                                            $prd_run_stt = ProductionRuncardStation::where('production_runcard_id', $all_ProductionRuncard[$i]->id)->where('status', 1)->orderBy('id','DESC')->get();
                                            for ($j=0; $j < count($prd_run_stt); $j++)
                                                $ttl_po_qty += $prd_run_stt[$j]->qty_output;
                                                // $ttl_po_qty += $prd_run_stt[$j]->qty_input;
                                        }
                                        ...
                                        $ttl_input = 0;
                                        $prd_run_stt = ProductionRuncardStation::where('production_runcard_id', $request->txt_prod_runcard_id_query)->where('status', 1)->get();
                                        for ($i=0; $i < count($prd_run_stt); $i++)
                                            $ttl_input += $prd_run_stt[$i]->qty_output;
                                            // $ttl_input += $prd_run_stt[$i]->qty_input;


                                        // return response()->json(['error_msg' => '2 / ' . (int)$request->qty_input . ' - ' . $dvc[0]->boxing]);


                                        // if( ( $ttl_input + (int)$request->qty_input ) > $dvc[0]->boxing )
                                        // return $ttl_input . ' ' . (int)$request->qty_input . ' ' . $dvc[0]->ship_boxing;
                                        if( ( $ttl_input + (int)$request->qty_input ) > $dvc[0]->ship_boxing )
                                            return response()->json(['error_msg' => 'You exceeding the total input per lot.']);
                                            // return 'You exceeding the total input per lot.';
                            ...
                            // if($validator->passes()){
                                ...
                                if(count($selected_prod_runcard) > 0){
                                    ...
                                    try {
                                        ...
                                        // return $final_runcard_no;
                                        $prd_runcard = ProductionRuncard::where('id', $selected_prod_runcard[$outer_index])->orderBy('id','DESC')->get();
                                        $prd_runcard_device_name = $prd_runcard[0]->device_name;
                                        $prd_runcard_device_name = CommonController::getInstance()->validate_device_name($prd_runcard_device_name);
                                        //TODO: MIGZ 09/04/24 Catch the error if the Matrix need to be change.
                                        $dvc = Device::where('name', $prd_runcard_device_name)->where('status',1)->get();
                                        if( count($dvc) == 0 ){
                                            DB::rollback();
                                            return response()->json([
                                                "result" => 0,
                                                "error_msg" => "Please check the matrix, then add or edit the process 2!",
                                            ]);
                                        }
                                        // $first_prd_runcard = ProductionRuncard::where('po_no', $prd_runcard[0]->po_no)->get();
                                        $ttl_po_qty = 0;
                                        ...
                                        $ttl_input = 0;
                                        $prd_run_stt = ProductionRuncardStation::where('production_runcard_id', $selected_prod_runcard[$outer_index])->where('status', 1)->get();
                                        for ($i=0; $i < count($prd_run_stt); $i++)
                                            $ttl_input += $prd_run_stt[$i]->qty_output;
                                            // $ttl_input += $prd_run_stt[$i]->qty_input;


                                        // return response()->json(['error_msg' => '3 / ' . (int)$request->qty_input . ' - ' . $dvc[0]->boxing]);
                        ...
                        if($validator->passes()){
                            ...
                            // if($user_info != null){
                                ...
                                if($prod_runcards->count() > 0){
                                    ...
                                    $final_runcard_no = 'LOT-0001';
                                }

                                // return $final_runcard_no . '<>4';
