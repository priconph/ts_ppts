...
use App\Model\YeuKitting;//tbl_wbs_material_kitting

class ProductionRuncardController_rev extends Controller
{
    //
 ...
 //            ->make(true);
                        ...
                        if( isset($request->txt_prod_runcard_id_query) ){
                            ...
                            try {
                                ...
                                if($prod_runcards->count() > 0){
                                    ...
                                    $explode_runcard_no[count($explode_runcard_no)-1] = str_pad($explode_runcard_no[count($explode_runcard_no)-1] + 1,3,"0", STR_PAD_LEFT);
    ...
    public function fn_select_prod_runcard_station_details(Request $request){
        ...
        $prod_runcard_stations = ProductionRuncardStation::with([
                'station' => function($query){
                    $query->where('status', 1);
                },
                'sub_station' => function($query){
            ...
            // ->where('production_runcard_id', $request->prod_runcard_id_query )
            ->where('id', $request['col_prod_runcard_station_id'])
            ->first();

        return response()->json(['prod_runcard_stations' => $prod_runcard_stations]);
    ...
    public function get_prod_runcard_by_po(Request $request){
        ...
        return DataTables::of($prod_runcards)
            ...
            ->addColumn('raw_checkbox', function($prod_runcard){
                $result = '';
                $disabled = '';
                if($prod_runcard->status == 3){
                    $disabled = 'disabled';
                }
                $result = '<center><input type="checkbox" production-runcard-id="' . $prod_runcard->id . '" class="chkSelProdRuncard" ' . $disabled . '></center>';

                return $result;
            })
            ->addColumn('raw_qty_output_sum', function($prod_runcard){
                $col_prod = '';
                if(count($prod_runcard->prod_runcard_station_many_details) > 0){
                    $col_prod = collect($prod_runcard->prod_runcard_station_many_details)->sum('qty_output');
                }
                return $col_prod;
            })
            ->addColumn('raw_qty_ng_sum', function($prod_runcard){
                $col_prod = '';
                if(count($prod_runcard->prod_runcard_station_many_details) > 0){
                    $col_prod = collect($prod_runcard->prod_runcard_station_many_details)->sum('qty_ng');
                }
                return $col_prod;
            })
            ->rawColumns(['raw_action', 'raw_status', 'total_no_of_ng', 'last_runcard_po_qty', 'raw_checkbox', 'raw_qty_output_sum'])
            ->make(true);
    }
    ...
    //     if($material_kitting != null){
    //         $doc_details_query = RapidActiveDocs::where('series', 'LIKE', '%' . $material_kitting->device_name . '%')
    //             ->get();
    //         $doc_a_drawing_query = collect($doc_details_query)->where('doc_type', 'A Drawing')->flatten(1);
    //         $doc_g_drawing_query = collect($doc_details_query)->where('doc_type', 'G Drawing')->flatten(1);
    //         $doc_kl_drawing_query = collect($doc_details_query)->where('doc_type', 'G Drawing')->flatten(1);
    ...
    public function get_wbs_material_kitting_rev(Request $request){
        ...
        // return $device_name_print;
        // return response()->json(['material_kitting' => $material_kitting]);
        ...
        if($material_kitting != null){
            ...
            $doc_orig_a_drawing_query = $doc_details_query->filter(
                        function($item) {
                            return (($item->doc_type=='A Drawing')||($item->doc_type=='AA Drawing')||($item->doc_type=='KL Drawing') || ($item->doc_type=='YEU Drawing'));
                        })
                        ->flatten(1);

            $doc_a_drawing_query = $doc_details_query->filter(
                        function($item) {
                            return (($item->doc_type=='KS Drawing')||($item->doc_type=='AC Drawing')||($item->doc_type=='BA Drawing')||($item->doc_type=='GK Drawing')||($item->doc_type=='GJ Drawing'));
                        })
                        ->flatten(1);

            $doc_g_drawing_query = $doc_details_query->filter(
                        function($item) {
                            return (($item->doc_type=='G Drawing')||($item->doc_type=='AG Drawing')||($item->doc_type=='B Drawing')||($item->doc_type=='BG Drawing') );
                        })
                        ->flatten(1);

            $doc_jrdjksdcgj_drawing_query = $doc_details_query->filter(
                function($item) {
                    return (($item->doc_type=='J Drawing')||($item->doc_type=='R Drawing')||($item->doc_type=='DJ Drawing')||($item->doc_type=='KS Drawing')||($item->doc_type=='DC Drawing')||($item->doc_type=='GJ Drawing'));
                })
                ->flatten(1);

            $doc_gpmd_drawing_query = $doc_details_query->filter(
                function($item) {
                    return (($item->doc_type=='Packing Material Specs (PMAT)')||($item->doc_type=='Packing Material'));
                })
                ->flatten(1);


            $query = RapidActiveDocs::where('series', 'LIKE', '%' . $device_name_print . '%');
            if( count($doc_g_drawing_query) == 0 ){
    ...
    // public function get_drawingno_qr(Request $request){
    //     $a_drawing_no = '';
    //     $g_drawing_no = '';
    //     $o_drawing_no = '';

    //     $a_drawing_no = QrCode::format('png')
    //                     ->size(200)->errorCorrection('H')
    //                     ->generate($request->a_drawingno);

    //     $a_drawing_no = "data:image/png;base64," . base64_encode($a_drawing_no);

    //     $g_drawing_no = QrCode::format('png')
    //                     ->size(200)->errorCorrection('H')
    //                     ->generate($request->g_drawingno);

    //     $g_drawing_no = "data:image/png;base64," . base64_encode($g_drawing_no);

    //     if( $request->o_drawingno ){

    //         $o_drawing_no = QrCode::format('png')
    //                         ->size(200)->errorCorrection('H')
    //                         ->generate($request->o_drawingno);

    //         $o_drawing_no = "data:image/png;base64," . base64_encode($o_drawing_no);
    //     }
    //     return response()->json(['a_drawing_no_qr' => $a_drawing_no, 'g_drawing_no_qr' => $g_drawing_no, 'o_drawing_no_qr' => $o_drawing_no]);
    //     // return response()->json(['a_drawing_no_qr' => $a_drawing_no, 'g_drawing_no_qr' => $g_drawing_no]);
    // }
    ...
    public function delete_runcard_accessory(Request $request)
    {
        ProdRuncardAccessory::where('id', $request->id)
            ->update([
                'status' => 2,
    ...
    public function check_pilot_ran_by_po_no(Request $request){
        $have = false;
        if( count( ProductionRuncard::where('po_no', $request->po_no)->where('have_pilot_run', 1)->get() ) > 0 )
            $have = true;
        return response()->json(['result' => $have]);
    }

    public function set_have_pilot_ran(Request $request){
        ProductionRuncard::where('id', $request->id)
            ->update([ 'have_pilot_run' => 1 ]);
        return response()->json(['result' => 1]);
    }
    ...
    public function edit_prod_runcard_station(Request $request){
        ...
        if((isset($request->txt_prod_runcard_id_query) || $request->txt_prod_runcard_id_query != null || $request->txt_prod_runcard_id_query != '')){
            if(isset($request->txt_prod_runcard_station_id_query)){
                $validator = Validator::make($data, [
                    'txt_prod_runcard_id_query' => ['required'],
                    'txt_prod_runcard_station_id_query' => ['required'],
                    'qty_input' => ['required'],
                            ...
                            if( $request->txt_prod_runcard_station_id_query ){
                                ...
                                // if($validator->passes()){
                                    ...
                                    try {
                                        ProductionRuncardStation::where('id',$request->txt_prod_runcard_station_id_query)
                                            ->update(
                                                [
                                                    ...
                                                    'terminal_area'=>$request->terminal_area,
                                                    // 'status' => 1,
                                        ...
                                        // angelo
                                        // Save Operators
                                        // ProductionRuncardStationOperator::where('production_runcard_station_id', $request->txt_prod_runcard_station_id_query)
                                        //     ->delete();

                                        // if(isset($request->operators_id)){
                                        //     for($index = 0; $index < count($request->operators_id); $index++){
                                        //         ProductionRuncardStationOperator::insert([
                                        //             'production_runcard_id' => $request->txt_prod_runcard_id_query,
                                        //             'production_runcard_station_id' => $request->txt_prod_runcard_station_id_query,
                                        //             'operator_id' => $request->operators_id[$index],
                                        //             'ct_area' => $request->ct_area[$index],
                                        //             'terminal_area' => $request->terminal_area[$index],
                                        //         ]);
                                        //     }
                                        // }
                                        ...
                                        // angelo
                                        // Save Machines
                                        ProductionRuncardStationMachine::where('production_runcard_station_id', $request->txt_prod_runcard_station_id_query)
                                            ->delete();
                                        ...
                                        ProductionRuncardStationMOD::where('production_runcard_station_id', $request->txt_prod_runcard_station_id_query)
                                            ->delete();
                                        // if has NG only
                                        if($request->qty_ng > 0){
                                            // Required to save MOD if has NG only
                    ...
                    //     if($user[0]->position == 4){
                            // if( $request->txt_prod_runcard_station_id_query ){
                                ...
                                // if($validator->passes()){
                                    ...
                                    try {
                                        $runcard_no = substr($request->txt_po_number, 0, -5);
                                        while ( $runcard_no[strlen($runcard_no)-1] == '-' )
                                            $runcard_no = substr($runcard_no, 0, -1);

                                        // Generate Runcard No
                                        $prod_runcards = ProductionRuncardStation::where('runcard_no', 'LIKE', $runcard_no . "%")->orderBy('id', 'desc')
                                        ->where('production_runcard_id', $request->txt_prod_runcard_id_query,)
                                        ->limit(1)->get();
                                        ...
                                        if($prod_runcards->count() > 0){
                                            ...
                                            // $final_runcard_no = substr($request->txt_po_number, 0, -5) . '-001';
                                            $final_runcard_no = $runcard_no . '-001';
                                        }
                                        return $final_runcard_no;
        ...
        else{ // For Multiple Runcard
            $validator = Validator::make($data, [
                // 'txt_prod_runcard_id_query' => ['required'],
                'selected_prod_runcard' => ['required'],
                'qty_input' => ['required'],
                'qty_output' => ['required'],
                'qty_ng' => ['required'],
                'txt_employee_number_scanner' => ['required'],
                    ...
                    if($user[0]->position == 4){
                        // if( $request->txt_prod_runcard_station_id_query ){
                            ...
                            // if($validator->passes()){
                                ...
                                $selected_prod_runcard = explode(',', $request->selected_prod_runcard);
                                // return 'wew';
                                // return count($selected_prod_runcard);
                                if(count($selected_prod_runcard) > 0){
                                    // return 'wew';
                                    // return $selected_prod_runcard;
                                    $counter = 0;
                                    try {
                                        ProductionRuncard::whereIn('id', $selected_prod_runcard)
                                            ->update(
                                                [
                                                    'status'=> 3,
                                                    'last_updated_by' => $user[0]->id,
                                        ...
                                        // return $selected_prod_runcard;
                                        for ($outer_index = 0; $outer_index < count($selected_prod_runcard); $outer_index++) {
                                            $counter++;

                                                $last_prod_runcard_station = ProductionRuncardStation::where('production_runcard_id', $selected_prod_runcard[$outer_index])->orderBy('id', 'desc')->first();
                                                ...
                                                $prod_runcard_station_id = ProductionRuncardStation::insertGetId([
                                                    'production_runcard_id'=> $selected_prod_runcard[$outer_index],
                                                    // 'mat_proc_id'=> $material_process[$index]->id,
                                                    // 'station_id'=> $material_process[$index]->station_sub_station->station_id,
    ...
    /* public function save_prod_runcard_details_rev(Request $request){ // Live Prod Runcard - FOR MANUAL AND AUTOMATIC RUNCARD
        date_default_timezone_set('Asia/Manila'); //txt_lot_no 'lot_no'
            ...
            if(Auth::check()){
                ...
                if(isset($request->txt_prod_runcard_id_query)){
                    ...
                    $rules = [
                        ...
                        // 'txt_pair_no' =>  'required',
                    ];

                    $messages = [
                        // 'txt_employee_number_scanner.required' => 'Employee No. field is required.',
                        'txt_po_number.required' => 'PO No. field is required.',
                        'txt_lot_qty.required' => 'Lot Quantity field is required.',
                        'setup_qualification.required' => 'Setup Qualification field is required.',
                        'sel_assembly_line.required' => 'Assembly Line field is required.',
                    ];
                    ...
                    try {
                        if($validator->passes()){
                            ...
                            // $user_info = User::where('employee_id', $request->txt_employee_number_scanner)
                            //     ->where('status', 1)
                            //     ->first();
                            ...
                            // if($user_info != null){
                                ...
                                // if(isset($request->setup_qualification)){
                                //     $setup_qualification = $request->setup_qualification;
                                // }
                                // if(isset($request->eng_qualification_id)){
                                //     $eng_qualification_id = $request->eng_qualification_id;
                                // }
                                // // if(isset($request->setup_qualified)){
                                // //     $setup_qualified = $request->setup_qualified;
                                // // }
                                // if(isset($request->qc_qualification)){
                                //     $qc_qualification = $request->qc_qualification;
                                // }
                                // if(isset($request->qc_stamp_id)){
                                //     $qc_stamp_id = $request->qc_stamp_id;
                                // }
                                // if(isset($request->qc_qualified)){
                                //     $qc_qualified = $request->qc_qualified;
                                // }
                                ...
                                ->update(
                                    [
                                        ...
                                        // 'eng_qualification_id' => $eng_qualification_id,
                                        // // 'setup_qualified' => $setup_qualified,
                                        // 'qc_qualification' => $qc_qualification,
                                        // 'qc_stamp_id' => $qc_stamp_id,
                                        // 'qc_qualified' => $qc_qualified,
                                        ...
                                        'assembly_line_id'=> $request->sel_assembly_line,
                                        'a_drawing_no'=> $request->txt_Adrawing_no,
                                        'a_revision'=> $request->a_revision,
                                        'g_drawing_no'=> $request->txt_Gdrawing_no,
                                        'g_revision'=> $request->g_revision,
                                        // 'o_drawing_no'=> $request->txt_Odrawing_no,
                                        // 'o_revision'=> $request->o_revision,
                                        // 'other_docs_no'=> $request->txt_other_docs_no,
                                        // 'other_docs_rev'=> $request->txt_other_docs_rev,
                                        'last_updated_by' => Auth::user()->id,
                                        'updated_at' => date('Y-m-d H:i:s')
                                ...
                                DB::commit();
                                return response()->json(['auth' => 1, 'result' => 1, 'error' => null, 'prod_runcard_id' => $request->txt_prod_runcard_id_query]); // Success
                            // }
                            // else{
                            //     return response()->json(['auth' => 1, 'result' => 2, 'error' => null]); // Invalid Operator
                            // }
                        }
                        else{
                ...
                // Add Runcard
                    ...
                    $messages = [
                        ...
                        // 'txt_pair_no.required' => 'Pair No. field is required.',
                    ];
                    ...
                    // try {
                        if($validator->passes()){
                            ...
                            // if($user_info != null){
                                // $runcard_no = substr($request->txt_po_number, 0, -5);
                                $PO = $request->txt_po_number;
                                $POnum = strlen($PO);
                                $substring = 'PO';
                                // return $PO;
                                // return 'asd';
                                if ($POnum == 7){ //TODO: nmodify Example: 0645LOT-001
                                    $PO_num = substr($PO, 4); //09122024 by Nessa
                                }else{
                                    // $PO_num = substr($PO, 5, 5); //09122024 by Nessa commented 11/21/2024 BiniManoy
                                    if (strpos($PO, $substring) !== false) { // added 11/21/2024 BiniManoy
                                        $PO_num = substr($PO, 4,4);
                                ...
                                // return $PO_num;
                                // $runcard_no = 'LOT';
                                $runcard_no = $PO_num.'LOT'; //09122024 by Nessa
                                //  return $runcard_no;
                                $prod_runcard_id = "";
                                ...
                                // Generate Runcard No
                                $prod_runcards = ProductionRuncard::where('po_no', $request->txt_po_number)->orderBy('id', 'desc')
                                    ->where('lot_no', '!=', null)
                                    ->limit(1)->get();
                                ...
                                if($prod_runcards->count() > 0){
                                    ...
                                    // $final_runcard_no = 'LOT-001';
                                    $final_runcard_no = $PO_num.'LOT-001'; //10252021
                                }
                                ...
                                $prod_runcard_id = ProductionRuncard::insertGetId([
                                    ...
                                    'orig_a_drawing_no'=> $request->txt_orig_Adrawing_no,
                                    'orig_a_revision'=> $request->orig_a_revision,
                                    ...
                                    'wi_d'=> $request->txt_WIDoc,
                                    'wi_d_revision'=> $request->txt_WIDoc_rev,

                                    'ogm_vig_ig_d'=> $request->txt_OGM_VIG_IGDoc,
                                    'ogm_vig_ig_d_revision'=> $request->txt_OGM_VIG_IGDoc_rev,

                                    'pp_d'=> $request->txt_PPDoc,
                                    'pp_d_revision'=> $request->txt_PPDoc_rev,

                                    'ud_d'=> $request->txt_UDDoc,
                                    'ud_d_revision'=> $request->txt_UDDoc_rev,

                                    'pm'=> $request->txt_PMDoc,
                                    'pm_revision'=> $request->txt_PMDoc_rev,

                                    'j_r_dj_ks_dc_gj'=> $request->txt_JRDJKSDCGJDoc_no,
                                    'j_r_dj_ks_dc_gj_revision'=> $request->txt_JRDJKSDCGJDoc_revision,

                                    'gp_md'=> $request->txt_GPMDDoc_no,
                                    'gp_md_revision'=> $request->txt_GPMDDoc_revision,
                                ...
                                DB::commit();
                                return response()->json(['auth' => 1, 'result' => 1, 'error' => null, 'prod_runcard_id' => $prod_runcard_id]); // Success
                            // }
                            // else{
                            //     return response()->json(['auth' => 1, 'result' => 2, 'error' => null]); // Invalid Operator
                            // }
                            ...
                            return response()->json(['auth' => 1, 'result' => 0, 'error' => $validator->messages()]); // Failed
                        }
                    // }
                    // catch(\Exception $e) {
                    //     DB::rollback();
    ...
    public function save_prod_runcard_details_rev(Request $request){ // ETO UNG LIVE NA CONTROLLER 04072025 by Nessa
        // return working function;
        date_default_timezone_set('Asia/Manila');
                ...
                // Add Runcard
                    ...
                    DB::beginTransaction();
                        if($validator->passes()){
                                ...
                                $PO_num = substr($request->txt_po_number, 5, 5); //10252021
                                ...
                                if($prod_runcards->count() > 0){
                                    ...
                                    $final_runcard_no = $PO_num.' LOT-001';
                                }
        ...
        if($request->ajax()){
            if(Auth::check()){
                ...
                $messages = [
                    ...
                    // 'setup_qualification.required' => 'Setup Qualification field is required.',
                    // 'txt_pair_no.required' => 'Pair No. field is required.',
                    'txt_prod_runcard_id_query.txt_prod_runcard_id_query' => 'Production Runcard ID field is required.',
                ];
                ...
                try {
                    if($validator->passes()){
                        ...
                        DB::commit();
                        return response()->json(['auth' => 1, 'result' => $result, 'error' => null, 'prod_runcard_id' => $request->txt_prod_runcard_id_query]);

                        // if($user_info != null){

                        //     $prod_runcard_info = ProductionRuncard::where('id', $request->txt_prod_runcard_id_query)->first();

                        //     if($prod_runcard_info != null){
                        //         $error_counter = 0;
                        //         if($prod_runcard_info->setup_qualification == 1){
                        //             // if($prod_runcard_info->setup_qualified == 0){
                        //             //     $error_counter++;
                        //             // }
                        //             if($prod_runcard_info->eng_qualification_id == null){
                        //                 // $error_counter++; // Uncomment this to enable checking in technician
                        //             }
                        //         }

                        //         if($prod_runcard_info->qc_qualification == 1){
                        //             // if($prod_runcard_info->qc_qualification == 0){
                        //             //     $error_counter++;
                        //             // }
                        //             if($prod_runcard_info->qc_stamp_id == null){
                        //                 $error_counter++;
                        //             }
                        //         }
                        //         else{
                        ...
                        //         if($prod_runcard_info->setup_qualification == 0){
                        //             $check_runcard = ProductionRuncard::where('pair_no', $prod_runcard_info->pair_no)
                        //             ->where('po_no', $prod_runcard_info->po_no)
                        //             ->where('setup_qualification', 1)
                        //             ->where('eng_qualification_id', '!=', null)
                        //             ->orderBy('id', 'desc')
                        //             ->first();

                        //             if($check_runcard == null){
                        //                 // $error_counter++; // Uncomment this to enable checking in technician
                        //             }

                        //             // return $check_runcard;
                        //         }

                        //         if($prod_runcard_info->qc_qualification == 0){
                        //             $check_runcard = ProductionRuncard::where('pair_no', $prod_runcard_info->pair_no)
                        //             ->where('po_no', $prod_runcard_info->po_no)
                        //             ->where('qc_qualification', 1)
                        //             ->where('qc_qualified', 1)
                        //             ->where('qc_stamp_id', '!=', null)
                        //             ->orderBy('id', 'desc')
                        //             ->first();
                        ...
                        //     }
                        //     else{
                        //         $result = 3; // Runcard not found
                        //     }


                        //     DB::commit();
                        //     return response()->json(['auth' => 1, 'result' => $result, 'error' => null, 'prod_runcard_id' => $request->txt_prod_runcard_id_query]); // Success
                        // }
                        // else{
                        //     return response()->json(['auth' => 1, 'result' => 2, 'error' => null]); // Invalid Operator
                        // }
    ...
    public function delete_runcard_station(Request $request){
        date_default_timezone_set('Asia/Manila');
        session_start();
                ...
                try {
                    if($validator->passes()){
                        ...
                        try {
                            ...
                            $user = User::where('employee_id', $request->employee_no)->where('status', 1)->get();

                            if($user->count() > 0){
                                ProductionRuncardStation::where('id', $request->runcard_station_id)
                                    ->where('status', 1)
                                    ->update([
                                        'status' => 2,
                                        'last_updated_by' => Auth::user()->id,
                                ...
                                // $runcard_station_info = ProductionRuncardStation::where('id', $request->runcard_station_id)->first();

                                // // return response()->json(['auth' => 1, 'result' => 0, 'error' => null, 'runcard_station_info' => $runcard_station_info]);

                                // if($runcard_station_info != null){
                                //     $runcard_stations = ProductionRuncardStation::where('step_num', '>', $runcard_station_info->step_num)
                                //     ->where('status', 1)
                                //     ->where('production_runcard_id', $runcard_station_info->production_runcard_id)
                                //     ->get();

                                //     // return response()->json(['auth' => 1, 'result' => 0, 'error' => null, 'runcard_stations' => $runcard_stations]);

                                //     for($index = 0; $index < count($runcard_stations); $index++){
                                //         ProductionRuncardStation::where('id', $runcard_stations[$index]->id)
                                //             ->where('status', 1)
                                //             ->decrement('step_num', 1);
                                //     }
                                // }
                                // else{
                                //     DB::rollback();
                                //     return response()->json(['auth' => 1, 'result' => 3, 'error' => null]);
                                // }
                            ...
                            DB::rollback();
                            return response()->json(['auth' => 1, 'result' => 0, 'error' => null]);
                        }
                    }
                    else{
                        return response()->json(['auth' => 1, 'result' => 0, 'error' => $validator->messages()]);
            ...
            if(Auth::check()){
                ...
                if(!isset($request->accessory_id)){
                    ...
                    $rules = [
                        ...
                        'issuance_id' => 'required',
                        'quantity' => 'required',
                        'usage_per_socket' => 'required',
                        // 'counted_by' => 'required',
                        // 'counted_by_date' => 'required',
                        // 'checked_by' => 'required',
                        // 'checked_by_date' => 'required',
                        // 'prod_supervisor' => 'required',
                    ];

                    $require_supervisor = 0;
                    if(isset($request->require_supervisor)){
                        $rules['prod_supervisor'] = 'required';
                        $rules['prod_supervisor_date'] = 'required';
                        $require_supervisor = 1;
                    }
                    ...
                    try {
                        if($validator->passes()){
                            $issuance_id = explode('--', $request->issuance_id)[2];
                            $item_code = explode('--', $request->issuance_id)[0];
                            $item_desc = explode('--', $request->issuance_id)[1];


                            ProdRuncardAccessory::insert([
                                'prod_runcard_id' => $request->prod_runcard_id,
                                'issuance_id' => $issuance_id,
                                'item_code' => $item_code,
                                'item_desc' => $item_desc,
                                'quantity' => $request->quantity,
                                'usage_per_socket' => $request->usage_per_socket,
                                'counted_by' => $request->counted_by,
                                'counted_by_date' => $request->counted_by_date,
                                'checked_by' => $request->checked_by,
                                'checked_by_date' => $request->checked_by_date,
                                'require_supervisor' => $require_supervisor,
                                'prod_supervisor' => $request->prod_supervisor,
                                'prod_supervisor_date' => $request->prod_supervisor_date,
                                'status' => 1,
                                'created_by' => Auth::user()->id,
                                'last_updated_by' => Auth::user()->id,
                                'created_at' => date('Y-m-d H:i:s'),
                ...
                // Edit ReferenceType
                    ...
                    try {
                        if($validator->passes()){
                            ...
                            ProdRuncardAccessory::where('id', $request->accessory_id)
                                ->update([
                                    'issuance_id' => $issuance_id,
                                    'item_code' => $item_code,
                                    'item_desc' => $item_desc,
    ...
    public function get_accessory_by_id(Request $request){
        date_default_timezone_set('Asia/Manila');
        $accessory_info = ProdRuncardAccessory::where('id', $request->accessory_id)
                            ->first();

        return response()->json(['accessory_info' => $accessory_info]);
    }

    public function view_accessories(Request $request){
        if($request->ajax()){
            $data = ProdRuncardAccessory::where('status', 1)
                        ->where('prod_runcard_id', $request->prod_runcard_id)
                        ->with([
                            // 'operator_info',
                            'counted_by_info',
                            'checked_by_info',
                            'prod_supervisor_info',
                        ])
                        ->get();
            ...
            return DataTables::of($data)
                ...
                ->addColumn('raw_counted_by_info', function($row){
                    $result = "";

                    if($row->counted_by_info != null){
                        $result .= $row->counted_by_info->name . ' / ';
                    }
                    else {
                        $result .= 'N/A / ';
                    }

                    if($row->counted_by_date != null){
                        $result .= $row->counted_by_date;
                    }
                    else {
                        $result .= 'N/A';
                    }
                ...
                ->addColumn('raw_checked_by_info', function($row){
                    $result = "";

                    if($row->checked_by_info != null){
                        $result .= $row->checked_by_info->name . ' / ';
                    }
                    else {
                        $result .= 'N/A / ';
                    }

                    if($row->checked_by_date != null){
                        $result .= $row->checked_by_date;
                    }
                    else {
                        $result .= 'N/A';
                    }
                ...
                ->addColumn('raw_prod_supervisor_info', function($row){
                    $result = "";

                    if($row->prod_supervisor_info != null){
                        $result .= $row->prod_supervisor_info->name . ' / ';
                    }
                    else {
                        $result .= 'N/A / ';
                    }

                    if($row->prod_supervisor_date != null){
                        $result .= $row->prod_supervisor_date;
                    }
                    else {
                        $result .= 'N/A';
                    }
                ...
                ->addColumn('raw_action', function($row){
                    $result = '';
                    if($row->status == 1){
                        $result .= '<button style="width:30px; margin-top: 3px;" type="button" class="btn btn-sm py-0 btn-info table-btns btnEditAccessory" accessory-id="' . $row->id . '"><i class="fa fa-edit" title="Edit"></i></button>';

                        $result .= ' <button style="width:30px; margin-top: 3px;" type="button" class="btn btn-sm py-0 btn-danger table-btns btnAccessoryActions" status="2" accessory-id="' . $row->id . '" title="Delete"><i class="fa fa-times"></i></button>';
                    }
                    else{
                        $result .= ' <button style="width:30px; margin-top: 3px;" type="button" class="btn btn-sm py-0 btn-success table-btns btnAccessoryActions" action="1" status="1" accessory-id="' . $row->id . '" title="Restore"><i class="fa fa-unlock"></i></button>';
                    }

                    return $result;
                })
                ->addColumn('raw_checkbox', function($row){
                    $result = '';
                    if($row->status == 1){
                        ...
                        if($row->counted_by_info != null){
                            $counted_by .= $row->counted_by_info->name;
                        }
                        else {
                            $counted_by .= '';
                        }

                        $result .= '<input style="width:30px; margin-top: 3px;" type="checkbox" class="btnCheckAccessory"
                        ' . ' accessory-id="' . $row->id . '" ' . '
                        ' . ' accessory-name="' . $row->item_desc . '" ' . '
                        ' . ' quantity="' . $row->quantity . '" ' . '
                        ' . ' usage-per-socket="' . $row->usage_per_socket . '" ' . '
                        ' . ' counted-by="' . $counted_by . '" ' . '
                        ' . ' counted-by-date="' . $row->counted_by_date . '" ' . '
                        value="' . $row->id . '">';
                ...
                ->rawColumns(['raw_status', 'raw_action', 'raw_checkbox'])
                ->make(true);
        }
        else{
            abort(403);
    ...
    public function check_lot_qty_before_submit_to_oqc_lot_app(Request $request)
    { //btnSubmitToOQCLotApp
        $ttl_qty = 0;
        $prd_run_stt = ProductionRuncardStation::where('production_runcard_id', $request->id)->where('status', 1)->get();
        for ($j=0; $j < count($prd_run_stt); $j++)
            $ttl_qty += $prd_run_stt[$j]->qty_output;

        $prd_runcard = ProductionRuncard::where('id', $request->id)->get();
        $prd_runcard = $prd_runcard[0]->device_name;

        //TODO: MIGZ 09-04-24 Remove extension (Ex. Burn-in & Test), if the Device Name not match in WBS Issuance & Kitting
        $prd_runcard = CommonController::getInstance()->validate_device_name($prd_runcard);

        $dvc = Device::where('name', $prd_runcard)->get();
        // return $dvc;
        if( $ttl_qty == $dvc[0]->ship_boxing )
            return response()->json(['question' => 'Do you want to submit this lot?']);
            return response()->json(['question' => 'Do you want to submit this with less than required lot qty?']);
    }

    public function get_first_lot_data_by_po_no(Request $request) //omodify
    {
        if( isset( $request->validate ) ){
            //TODO: MIGZ 09-04-24 Remove Burn-in & Test, if the Device Name in WBS Issuance & Kitting
            // return 'true';
            $prd_runcard = $request->device_name;
            $prd_runcard = CommonController::getInstance()->validate_device_name($prd_runcard);
            $device = Device::where('name', $prd_runcard)->get();
            // $ttl_lot_cnt = ceil((int)$request->po_qty / $device[0]->ship_boxing); // Orig
            // // return $ttl_lot_cnt;
            // if( count( ProductionRuncard::where('po_no', $request->po_no)->get() ) >= $ttl_lot_cnt )
            //     return response()->json(['error_msg' => 'You exceeding the maximum lot count.']);
            // return $prd_runcard;
        // return response()->json(['error_msg' => ProductionRuncard::where('po_no', $request->po_no)->orderBy('id')->limit(1)->get()]);
            $ttl_po_qty = 0;
            $all_ProductionRuncard = ProductionRuncard::where('po_no', $request->po_no)->get();
            for ($i=0; $i < count($all_ProductionRuncard); $i++) {
                $prd_run_stt = ProductionRuncardStation::where('production_runcard_id', $all_ProductionRuncard[$i]->id)->where('status', 1)->get();
                for ($j=0; $j < count($prd_run_stt); $j++)
                    $ttl_po_qty += $prd_run_stt[$j]->qty_output;
            }
            // if( count($all_ProductionRuncard)>0 && $ttl_po_qty == $all_ProductionRuncard[0]->po_qty )
            //     return response()->json(['error_msg' => 'Max PO quantity was already meet.']);

            $data = ProductionRuncard::where('po_no', $request->po_no)->where('status', 1)->get();
            // if( count($data)>0 )
        ...
        if( count($data)>0 ){
            ...
            //04082024 by Nessa
            $device_name_print = 'not found';
            if(isset($data[0]->device_name)){
                $device_name_print = $data[0]->device_name;
            ...
            //04082024 by Nessa
            // if( isset($data[0]->device_name) ){
            //     $data[0]->device_details = Device::where('name', $data[0]->device_name)->get();
            //     $data[0]->aseembly_line_details = AssemblyLine::find($data[0]->assembly_line_id);
            // }

            if( isset($device_name_print) ){
                $data[0]->device_details = Device::where('name', $device_name_print)->get();
                $data[0]->aseembly_line_details = AssemblyLine::find($data[0]->assembly_line_id);
            }

            // return $data[0]->device_details;
        }

        return response()->json(['data' => $data, 'po_no' => $request->po_no, 'runcard_id' => $request->runcard_id]);
    }

    public function check_max_lot_num_by_po_no(Request $request)
    {
    }

    public function checkQuantityOutputIsComplete(Request $request)
    {

        $runcard = ProductionRuncard::find($request->id);

        if( !isset($runcard->id) )
            return response()->json(['error_msg' => 'Lot info not found.', 'result' => 1]);

        // $device = Device::where('name', $runcard->device_name)->get(); //OLD 07-03-2024
        $device = Device::where('name', $runcard->device_name)->where('status', 1)->get(); // CHAN 07-03-2024
        // return $device[0]->ship_boxing;

        if( count($device) == 0 )
            return response()->json(['error_msg' => 'Device info not found.', 'result' => 1]);

        $lot_no = (int)(explode('-', $runcard->lot_no)[1]);
        $max_lot_no = ceil((int)($runcard->po_qty) / (int)($device[0]->ship_boxing));

        $add_in_output_qtt = ($lot_no - 1) * (int)($device[0]->ship_boxing);

        $output_qtt = 0;
        $stations = ProductionRuncardStation::where('production_runcard_id', $request->id)->where('status', 1)->get();
        for ($i=0; $i < count($stations); $i++)
            $output_qtt += $stations[$i]->qty_output;

        $is_disabled = 1;
        // return $output_qtt;
        if( $runcard->status == 1 ){
            // return $output_qtt;
            // if( $lot_no == $max_lot_no ){
            //     if( (int)($runcard->po_qty) == ($output_qtt + $add_in_output_qtt) )
            //         $is_disabled = 0;
            //     if( $output_qtt == (int)($device[0]->ship_boxing) )
            //         $is_disabled = 0;
            // }else{
            //     if( $output_qtt == (int)($device[0]->ship_boxing) )
            //         $is_disabled = 0;
        ...
        return response()->json(['result' => $is_disabled, '$output_qtt' => $output_qtt ]);
        // return response()->json(['result' => $is_disabled]);
    }

    public function updateOqcInspectIfViewDrawingAndScanTrays(Request $request)
    {
    ...
    {
        return response()->json(['data' => OQCInspection_2_ViewChecker::where('runcard_id', $request->id)->get()]);
    }
       ...
       $device = Device::where('name', $device_name_print)->where('status',1)->get();
        ...
        for ($i=1; $i <= $sticker_cnt; $i++) {
            ...
            $data[] = array(
                ...
                'qtt' => $qtt_tray,
                'counter' => ($lot_start_counter + $i) . '/' . ($lot_start_counter + $sticker_cnt), // Orig
                // 'counter' => ($lot_start_counter + $i + 1) . '/' . ($lot_start_counter + $sticker_cnt + 1), // Chris => please remove this after processing of 450246712800010
                'ctr' => $lot_start_counter,
                'count_per_tray' => $i,
                'stt' => 0,
            );
    ...
    public function getTotalQuantityByRuncard(Request $request)
    {
        $ttl_input = 0;
        $ttl_output = 0;
        $ttl_ng = 0;
        $result = ProductionRuncardStation::where('production_runcard_id', $request->lotapp_id)->where('status', 1)->get();
        for ($i=0; $i < count($result); $i++){
            $ttl_input = $ttl_input + $result[$i]->qty_input;
            $ttl_output = $ttl_output + $result[$i]->qty_output;
            $ttl_ng = $ttl_ng + $result[$i]->qty_ng;
        }

        return response()->json(['data' => array( 'ttl_input' => $ttl_input, 'ttl_output' => $ttl_output, 'ttl_ng' => $ttl_ng )]);
    }
