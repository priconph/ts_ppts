...
<!--             <div class="card card-primary">
                        ...
                        <table class="table table-bordered" style="width: 100%; font-size: 75%">
                            <thead>
                              <tr>
                                ...
                                <th style="padding: 5px; width: 15%;">Counter</th>
                                <th style="padding: 5px; width: 15%;">Status</th>
                                <th style="padding: 5px; width: 15%;">Unique Number</th>
                              </tr>
                            </thead>
            ...
            <div class="card card-primary">
              ...
              <h5 class="card-title"  id="edit_title_scan_casemark">2. Scan Casemark Details (QC)</h5>
               ...
               <div class="row">
                  <div class="col" id="div_show_packinglist_by_scanned">
                  </div>
                </div>
            ...
            <div class="card card-primary">
              <div class="card-header">
                <div id="div_fgs_endorsement">
                  <h5 class="card-title">3. FGS Endorsement (QC/Traffic) - Packinglist Details<button hidden type="button" id="btn_show_packing_list" class="btn btn-sm btn-primary">Show</button> </h5>
                  <br>
                  <br>
                   <div class="row">
                      <div class="col">
...
<!--           </div>
 -->
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-sm btn-success" id="btnCheckPackinglist">CheckPacking List</button>
...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    ...
    //         dlabel_to_scan[1] == data[1] &&
    //         dlabel_to_scan[2] == data[2]
    //         //&&  dlabel_to_scan[3] == data[3] //TODO: Check the validation
    //     )
    //     {
    ...
    $('#modal_scan_tray_qrcode_3').keypress(function(e) {
      if( e.keyCode == 13 && $('#modal_scan_tray_qrcode_3').val() !='' ){
        ...
        // console.log('datatoScan', data);

        // ADDED BiniManoy - 04/08/2025
        const unique_sticker_to_match = dlabel_to_scan[5];
        // const web_unique_sticker = data[3];

        let web_unique_sticker;

        //clark comment 12122025
        let get_po = data[0];
        // get_po = get_po.substring(18, 25);
        if(get_po.includes("PO")){
            web_unique_sticker = data[2];
        }else{
            web_unique_sticker = data[3];
        }
        //clark comment 12122025

        const unique_sticker = web_unique_sticker.substring(10, 25); // from index 10 to 25 (not inclusive)

        console.log('unique_sticker', unique_sticker);
        console.log('unique_sticker_to_match', unique_sticker_to_match);

        if(unique_sticker == unique_sticker_to_match){
        // if(unique_sticker_to_match == unique_sticker_to_match){ // with issue if PO starts with PO
          $('#tbl_dlabel_scan').css("background-color","#51FF51")
          $('#tray_check_list_id_3').html('scanned')
          $('#modal_scan_tray_3').modal('hide');
    ...
    //     // if( dlabel_to_scan[0] == data[0] && dlabel_to_scan[1] == (data[3]+' '+data[4]) && dlabel_to_scan[3] == data[5] ){
    //     // if( dlabel_to_scan[0] == data[0] && dlabel_to_scan[1] == (data[data.length-3]+' '+data[data.length-2]) && dlabel_to_scan[3] == data[data.length-1] ){
    //     if( dlabel_to_scan[0] == data[0] && dlabel_to_scan[2] == data[data.length-4] && dlabel_to_scan[1] == (data[data.length-3]+' '+data[data.length-2]) && dlabel_to_scan[3] == data[data.length-1] ){
    //       $('#tbl_dlabel_scan').css("background-color","#51FF51")
    //       $('#tray_check_list_id_3').html("scanned")
    //       dlabel_to_scan[4] = 1
    //       $('#modal_scan_tray_3').modal('hide');
    ...
    $('#modal_scan_tray_qrcode_4').keypress(function(e) {
      if( e.keyCode == 13 && $('#modal_scan_tray_qrcode_4').val() !='' ){
        ...
        //   $('#modal_scan_tray_notif1').modal('show')
            ...
            let notif_alert = `<p style="font-size:50px; color:red;">Invalid <b>FINAL PACKING</b> QR Code Details,</p>
                             <p style="font-size:50px; color:red;"><strong>CALL THE ATTENTION OF IMMEDIATE SUPERVISOR !</strong></p>`;
            $('#alert_notif').html(notif_alert);
            $('#modalScanEmployeeId').modal('show')
        }
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          // }
            ...
            let _found = false
              ...
              // if( !_found ){
              //   if( (data[i][data[i].length-5] + "" + data[i][data[i].length-4] + data[i][data[i].length-3] + data[i][data[i].length-2] + data[i][data[i].length-1]) == "2-129" ){
              //     indexes.push(i)
              //     _found = true
              //   }
              // }
        ...
        for (var i = 0; i < casemark_list.length; i++) {
          ...
          for (var j = 0; j < input_text_filtered.length; j++) {
            ...
            if( casemark_list[i][0] == input_text_filtered[j][0] && casemark_list[i][1] == input_text_filtered[j][1] &&
                casemark_list[i][2] == input_text_filtered[j][2] && casemark_list[i][3] == input_text_filtered[j][3] &&
                ship_to == _ship_to ){ // 10072023
                // ship_to == ship_to ){ // pag ayaw eto uncomment
                index_found = i
                break
              }
          ...
          if( index_found != null ){
            ...
            if( $('#edit_mode').val()=='trffic' )
              ...
              $('#btnCheckPackinglist').click()

            break
          }

        }
        console.log('index_found',index_found);
        if( index_found == null ){
            ...
            $('#modal_scan_tray_casemark').modal('hide')
            $('#modal_scan_tray_qrcode_casemark').val('')
            let notif_alert = `<p style="font-size:50px; color:red;">Invalid <b>CASEMARK</b> QR Code Details,</p>
                                <p style="font-size:50px; color:red;"><strong>CALL THE ATTENTION OF IMMEDIATE SUPERVISOR !</strong></p>`;
            $('#alert_notif').html(notif_alert);
            $('#modalScanEmployeeId').modal('show')
        }
    ...
    $('#btn_scan_tray_casemark').click(function() {
      // console.log('value', dlabel_to_scan[4]);
    ...
    $('#modal_scan_tray_qrcode').keypress(function(e) {
      if( e.keyCode == 13 && $('#modal_scan_tray_qrcode').val() !='' ){
        ...
        //   $('#modal_scan_tray_qrcode').focus()
            $('#modal_scan_tray').modal('hide')
            $('#modal_scan_tray_qrcode').val('')
            let notif_alert = `<p style="font-size:50px; color:red;">Invalid <b>TRAY</b> QR Code Details</p>
                                <p style="font-size:50px; color:red;"><strong>CALL THE ATTENTION OF IMMEDIATE SUPERVISOR !</strong></p>`;
            $('#alert_notif').html(notif_alert);
            $('#modalScanEmployeeId').modal('show')
        }
   ...
   $(document).on('click','.btn_final_inspection_traffic', function(){
    ...
    $('#btnCheckPackinglist').attr('hidden', true)
    isPassCheckPackinglist = true
    $('#div_show_packinglist_by_scanned').html('')
                ...
                $('#add_lot_qty').val(lotapp_quantity);
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        if( list[i]['box_no'].split('-').length == 2 ){
                            _qtt = list[i]['gross_weight'].split('/box')[0].split('(')
                            _qtt = _qtt[_qtt.length-1]
                          }
                    ...
                    _status = "pending"
                    if( JsonObject['final_packing_save_state'] == null ){
                        dlabel_to_scan = [ po_num, lot_number, lotapp_quantity, JsonObject['counter'], 0, JsonObject['unique_num'] ]
                    }else{
                        if( JsonObject['final_packing_save_state']['dlabel_ctr'] == JsonObject['counter'] ){
                            dlabel_to_scan = [ po_num, lot_number, lotapp_quantity, JsonObject['counter'], 1, JsonObject['unique_num'] ]
                            _style = "style='background-color: #51FF51;'"
                            _status = "scanned"
                        }else{
                            dlabel_to_scan = [ po_num, lot_number, lotapp_quantity, JsonObject['counter'], 0, JsonObject['unique_num'] ]
                    ...
                    html_3 +=     "<td style='padding: 5px; width: 15%;'>" + JsonObject['unique_num'] + "</td>" // added by Nessa 04072025
                    html_3 += "</tr>"
   ...
   $('#btnSubmitInspection').click(function(){
    ...
    if( $('#edit_mode').val()=='qc' ){
      if( !isPassCheckPackinglist ){
        alert( 'Total Quantity needed in Packing is not complete. Please complete first.' )
        return
      }
    }
   ...
   let isPassCheckPackinglist = true

   $('#btnCheckPackinglist').click(function(){
      ...
      $.ajax({
        ...
        success: function(data){
          ...
          if( data['shipments'] != null ){
            ...
            let packing_list = data['shipments']

            html += '<table class="table table-bordered" style="width: 100%; font-size: 75%">'
            html +=   '<tr>'
            html +=     '<th colspan="4" style="padding: 5px; text-align: center;">WED EDI Scanned</th>'
            html +=   '</tr>'
            html +=   '<tr>'
            html +=     '<th style="padding: 5px;">PO No.</th>'
            html +=     '<th style="padding: 5px;">Lot No.</th>'
            html +=     '<th style="padding: 5px; text-align: center;">Total Lot Qty</th>'
            html +=     '<th style="padding: 5px; text-align: center;">Counter</th>'
            html +=   '</tr>'

            let status
            for (var i = 0; i < packing_list.length; i++) {

              let balance = parseInt(packing_list[i]['total_qty'])-parseInt(packing_list[i]['prod_station_ttl'])
              status = "complete"
              html += "<tr style='background-color: #51FF51;'>"
              html +=     "<td style='padding: 5px;'>" + packing_list[i]['lot_details']['po_no'] + "</td>"
              html +=     "<td style='padding: 5px;'>" + packing_list[i]['lot_details']['lot_no'] + "</td>"
              html +=     "<td style='padding: 5px;'>" + packing_list[i]['total_qty'] + "</td>"
              html +=     "<td style='padding: 5px;'>" + packing_list[i]['wed_edi']['package_no'] + "</td>"
              html += "</tr>"
            }
          ...
          if( data['PackingListDetails'] != null ){

            let packing_list = data['PackingListDetails']
            html += '<table class="table table-bordered" style="width: 100%; font-size: 75%">'
            html +=   '<tr>'
            html +=     '<th style="padding: 5px;">Packing Ctrl No.</th>'
            html +=     '<th style="padding: 5px;">PO No.</th>'
            html +=     '<th style="padding: 5px; text-align: center;">Total Qty</th>'
            html +=     '<th style="padding: 5px; text-align: center;">Qty Scanned</th>'
            html +=     '<th style="padding: 5px; text-align: center;">Balance Qty</th>'
            html +=     '<th style="padding: 5px; text-align: center;">Status</th>'
            html +=   '</tr>'
            ...
            for (var i = 0; i < packing_list.length; i++) {
              ...
              if( 0 >= balance ){
                ...
                status = "pending"
                html += "<tr>"
                isPassCheckPackinglist = false
              }

              html +=     "<td style='padding: 5px;'>" + data['packing']['control_no'] + "</td>"
              html +=     "<td style='padding: 5px;'>" + packing_list[i]['po'] + "</td>"
              html +=     "<td style='padding: 5px; text-align: center;'>" + packing_list[i]['total_qty'] + "</td>"
              html +=     "<td style='padding: 5px; text-align: center;'>" + packing_list[i]['prod_station_ttl'] + "</td>"
              html +=     "<td style='padding: 5px; text-align: center;'>" + balance + "</td>"
              html +=     "<td style='padding: 5px; text-align: center;'>" + status + "</td>"
              html += "</tr>"
          ...
          $('#div_show_packinglist_by_scanned').html(html)
...
function _lcl_TSPTSSubmitFinalPackingInspection()
{
    ...
    $.ajax({
        ...
        method: "post",
        data: $('#formFinalPackingInspection').serialize() + '&packing_id=' + packing_id + '&box_no=' + box_no + '&po_no=' + po_no + '&qty=' + qty + '&wed_edi_id=' + wed_edi_id,
        dataType: "json",
        beforeSend: function(){
      ...
      if( e.keyCode == 13 && $('#txt_employee_id_2').val() !='' && ($('#txt_employee_id_2').val().length >= 4) ){
          ...
          $('#modalSearchInspector_2').modal('hide');
            ...
            $.ajax({
              ...
              {
                if(JsonObject['result'] == 1)
                  GetInspectorDetails_2(JsonObject['emp_id']);
                else if(JsonObject['result'] == 0)
                  toastr.error('Scanned Employee ID is not a QC Inspector.');
                else
                  toastr.error(JsonObject['error_msg']);
