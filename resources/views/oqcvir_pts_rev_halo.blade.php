...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    ...
    // nmodify - Search PO using object
    //JSON.parse($('#modal_scan_tray_qrcode').val());
  ...
  $(document).on('click','.btnView', function(){
    ...
    $('#modal_scan_tray_qrcode').keyup(function(e) {
      if( e.keyCode == 13 && $('#modal_scan_tray_qrcode').val() !='' ){
        ...
        console.log('modal_scan_tray_qrcode',data);
        console.log('tray_check_list',tray_check_list[i]);
   ...
   $(document).on('click','.btn-goto-wbsoqc', function(){
    ...
    $.ajax({
      ...
      success: function(JsonObject)
      {
        ...
        if( view ){
          $.ajax({
            url: "get_first_lot_data_by_po_no", // ProductionRuncardController_rev_old
            method: "get",
            data:
            {
              po_no: $('#id_po_no').val(),
            ...
            success: function(JsonObject)
            {
              if( JsonObject['data'].length > 0 ){
                ...
                // $("#modalRuncardDetails_device_name").val( data['device_name'] ) //commented 04282024 by nessa
                $("#modalRuncardDetails_device_name").val( data['device_details'][0]['name'] ) //new 04282024 by nessa
                ...
                $("#modalRuncardDetails_WIDoc_rev").val( (data['wi_d_revision'] != null && data['wi_d_revision'] != '') ? data['wi_d_revision'] : 'N/A' )

                $("#modalRuncardDetails_OGM_VIG_IGDoc").val( (data['o_drawing_no'] != null && data['o_drawing_no'] != '') ? data['o_drawing_no'] : 'N/A' )
                $("#modalRuncardDetails_OGM_VIG_IGDoc_rev").val( (data['ogm_vig_ig_d_revision'] != null && data['ogm_vig_ig_d_revision'] != '') ? data['ogm_vig_ig_d_revision'] : 'N/A' )
          ...
          $.ajax({
            ...
            success: function(JsonObject){
              ...
              window.open('http://192.168.180.11/pmi-subsystem/oqcinspection?' + encodeURI(
                ...
                '&ww=' + JsonObject['ww'] +
                '&app_date_time=' + JsonObject['app_date_time'] +
    ...
    $("#btnGotoPMIOQCInspection").click(function(){
        ...
        console.log(need_to_cherck_all_drawings);
 ...
 $('#btnSubmitEditInspection').click(function(){
  ...
  //- Search Drawing no
   ...
   $(document).on('keypress','#id_search_Drawing',function(e){
    ...
    $(document).on('keydown',function(e){
      if ($("#oqc_details_scan_id_id").is(":focus")) {
        setTimeout(function() {
          $("#oqc_details_scan_id_id").val('');
        }, 200);
      }
    ...
    $(document).on('keydown',function(e){
      if ($("#txt_employee_edit_id").is(":focus")) {
        setTimeout(function() {
          $("#txt_employee_edit_id").val('');
        }, 200);
      }
