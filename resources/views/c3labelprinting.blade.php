...
*/
  ...
  <div class="content-wrapper">
      ...
      <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
                ...
                <form id="frm_sticker_details">
                  ...
                  <div class="row">
                    ...
                    <div class="col-12 mt-3">
                      <button type="button" id="btn_td_print_for_box" class="btn btn-info btn-sm w-100"><i class="fa fa-print"></i> Print for box</button>
                    </div>
                  </div>
                </form>
              </div>
            ...
            <div class="row">
              <div class="col-4">
                <button type="button" hidden class="btn btn-sm btn-info" id="btn_history" title="Pint History"> <i class="fa fa-clock"></i> Print History</button>
              </div>
            </div>
          </div>
    ...
    <!-- Modal -->
    <div class="modal fade" id="mdl_confirm" tabindex="-1" role="dialog" aria-labelledby="cnptsmodal" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <div class="row">
              <div class="col-12">
                <div class="input-group input-group-sm mb-2">
                  ...
                  <textarea class="form-control" name="txt_confirm_remarks" id="txt_confirm_remarks"></textarea>
                </div>
              </div>
            </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-sm btn-success" id="btn_save_confirm">Confirm</button>
            <button type="button" class="btn btn-sm btn-secondary" data-dismiss="modal">Close</button>
          </div>
        </div>
      </div>
              ...
              <tbody>
                <tr>
                  <td colspan="2" class="text-bottom">
                    <img class="logo-left" src="http://rapidx/pats_ts/public/images/pricon-logo.png?v=2">
                  </td>
                  <td colspan="4" class="bold h5">Accessory Tag</td>
                </tr>
                <tr>
                  <td style="height: 4px; width: 16%">&nbsp;</td>
                  <td style="height: 4px; width: 16%"></td>
                  <td style="height: 4px; width: 16%"></td>
                  <td style="height: 4px; width: 16%"></td>
                  <td style="height: 4px; width: 16%"></td>
    ...
    $(document).ready(function () {
      ...
      dt_accessories      = $('#tbl_accessories').DataTable({
        "processing" : true,
          ...
          bAutoWidth: false,
          "columns":[
            { "data" : "raw_data.action", orderable:false, searchable:false, "width":"70px" },
            { "data" : "item_desc" },
            { "data" : "qty" },
            { "data" : "usage_qty" },
          ...
          info: false,
          "rowCallback": function(row,data,index ){
            if( $(row).html().toLowerCase().indexOf('is_deleted')>-1 ){
        ...
        if( ($("#mdl_employee_number_scanner").data('bs.modal') || {})._isShown ){
          ...
          if( e.keyCode == 13 ){
            ...
            if ( ( formid ).indexOf('#') > -1){
              ...
              switch( formid ){
                case 'fn_delete_accessory_item':
                  fn_delete_accessory_item();
                break;
...
//new
      ...
      function select_record(id){
        ...
        $('#btn_td_print_for_box').attr('disabled',true);
            ...
            success : function(data){
              if($.trim(data)){
                ...
                if(!data[0]['deleted_at']){
                  ...
                  $('#btn_td_print_for_box').attr('disabled',false);
      ...
      $('#btn_td_print_for_box').on('click',function(){
        var html_prod = '';
            html_prod += '      <tr>';
            html_prod += '        <td colspan="6" class="">Product Name: <u class="bold">'+$('#txt_print_device_name').val()+'</u></td>';
            html_prod += '      </tr>';
            html_prod += '      <tr>';
            html_prod += '        <td colspan="4" class="bold text-center h5">Accessory Name</td>';
            html_prod += '        <td colspan="1" class="bold text-center h5">QTY</td>';
            html_prod += '        <td></td>';
            html_prod += '      </tr>';

        var str = '';
        str += $('#txt_html_sticker_container_1').val();
        str += $('#txt_html_box_container_1').val();
        str += html_prod;

        $('#tbl_accessories tbody tr').each(function(){
          if( $(this).html().indexOf('is_deleted') < 0 ){
            str += $(this).find('.html_box_item').val();
          }
        });

        // str += $('#tbl_accessories tbody').find('tr:eq(0)').find('.html_box_item').val();
        str += $('#txt_html_box_container_2').val();
      ...
      $('#tbl_accessories tbody').on('click','.btn_td_print',function(){
        var str = '';
        str += $('#txt_html_sticker_container_1').val();
      ...
      $('#tbl_accessories tbody').on('click','.td_btn_delete',function(){
        $('#txt_confirm_remarks').val('');
      ...
      $('#btn_save_confirm').on('click',function(){
        //---
        var alert_msg = '';
        if( !$('#txt_confirm_remarks').val() ){
          alert_msg = 'Remarks is required.';
        }
        if(alert_msg){
          show_alert('<i class="fa fa-exclamation-triangle text-warning"></i>','Message',alert_msg,0);
          return;
        ...
        //---
        $('#txt_employee_number_scanner').val('');
        $('#mdl_employee_number_scanner').attr('data-formid','fn_delete_accessory_item').modal('show');
      });

      function fn_delete_accessory_item(){
        var data = {
          'data_id'                       : $('#tbl_accessories').find('tr.table-active').find('.td_id').val(),
          '_token'                        : '{{csrf_token()}}',
          'txt_confirm_remarks'           : $('#txt_confirm_remarks').val(),
          'txt_employee_number_scanner'   : $('#txt_employee_number_scanner').val(),
        };
        $.ajax({
          'data'      : data,
