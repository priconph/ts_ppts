...
@auth
          ...
          <div class="col-md-12" id="colDevice">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Device (Process / Packing Matrix)</h3>
              </div>
                    ...
                    <table id="tblDevices" class="table table-sm table-bordered table-striped table-hover" style="width: 100%;">
                      <thead>
                        <tr>
                          <th>Code</th>
                          <th>Product Name</th>
                          <th>Process - Overall Inspection</th>
                          <th>Qty per Tray/Box</th>
                          <th>Qty Per Lot/Master Box</th>
                          <th>Type</th>
                          <th>Label</th>
                          <th>Status</th>
                          <th>Action</th>
              ...
              <div class="col-sm-12">
                ...
                <div class="form-group">
                  <label>Device Name</label>
                    <input type="text" class="form-control" name="name" id="txtAddDeviceName" readonly>
                </div>

                <div class="form-group">
                  <label>Boxing/Tray</label>
                    <input type="text" value="0" min="0" class="form-control" name="boxing" id="txtAddDeviceBoxing" readonly>
                </div>

                <div class="form-group">
                  <label>Shipping Boxing</label>
                    <input type="text" value="0" min="0" class="form-control" name="ship_boxing" id="txtAddDeviceShipBoxing" readonly>
                </div>

                <div class="form-group">
                  <label>Process - Overall Inspection (0 - Without, 1 - With)</label>
                    <input type="text" value="0" min="0" class="form-control" name="process" id="txtAddPOI" readonly>
                </div>

                <div class="form-group">
                  <label>Type (1 - Burn-In Sockets, 2 - Micron PATS, 3 - Test Sockets)</label>
                    <input type="text" value="0" min="0" class="form-control" name="type" id="txtAddType" readonly>
                </div>

                <div class="form-group">
                  <label>Label (0 - No (Series Name), 1 - With (Series Name))</label>
                    <input type="text" value="0" class="form-control" name="label" id="txtAddLabel" readonly>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="modal-body">
            <div class="row">
              <div class="col-sm-12">
                ...
                <div class="form-group">
                  <label>Device Name</label>
                    <input type="text" class="form-control" name="name" id="txtEditDeviceName" readonly>
                </div>

                <div class="form-group">
                  <label>Boxing/Tray</label>
                    <input type="number" min="0" class="form-control" name="boxing" id="txtEditDeviceBoxing">
                </div>

                <div class="form-group">
                  <label>Shipping Boxing</label>
                    <input type="number" min="0" class="form-control" name="ship_boxing" id="txtEditDeviceShipBoxing">
                </div>

                <div class="form-group">
                  <label>Process - Overall Inspection (0 - Without, 1 - With)</label>
                    <input type="text" min="0" class="form-control" name="process" id="txtEditPOI">
                </div>

                <div class="form-group">
                  <label>Type (1 - Burn-In Sockets, 2 - Micron PATS, 3 - Test Sockets) </label>
                    <input type="text" min="0" class="form-control" name="type" id="txtEditType">
                </div>

                <div class="form-group">
                  <label>Label (0 - No (Series Name), 1 - With (Series Name))</label>
                    <input type="text" min="0" class="form-control" name="label" id="txtEditLabel">
                </div>
              </div>
            </div>
          </div>
  ...
  <!-- /.modal -->
   ...
   <!-- Modal -->
    <div class="modal fade" id="update_modal" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
      <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body pt-0">
            {{-- <div><img id="ts_ppts" loading="lazy" role="presentation" src="/ts_ppts/resources/content/matrix-update.gif"
            sizes="(max-width: 800px) 100vw,50vm"   /></div> --}}
            <div class="row  justify-content-center">
                <video controls autoplay muted class="mySlides" data-sec="350" width="700" height="350">
                    <source src="/ts_ppts/resources/content/matrix-update.mp4" type="video/mp4" />
                </video>
            </div>
          </div>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function () {
      ...
      dataTableDevices = $("#tblDevices").DataTable({
        ...
        $("#txtAddDeviceBarcode").change(function(e){
            let code = $(this).val();
            $.ajax({
                type: 'GET',
                url: `http://rapid/NAAYES/api/ypics_ts_ppts.php`,
                data: { 'barcode' : code },
                dataType: 'json',
                beforeSend: function(){
                },
                success: function (response) {
                    if(response.is_success === 'true'){
                        $('#txtAddDeviceName').val(response['NAME']);
                        toastr.success(`Valid Code !`);
                    }else{
                        // DITO KA MAGCCODE BUKAS DARREN GAGAWAN MO NG FUNCTION!!!
                          $.ajax({
                            type: "GET",
                            url: "get_device_from_yeu_kittings",
                            data: { 'barcode' : code },
                            dataType: "json",
                            ...
                            //   $('#txtAddDeviceName').val(result[0].product_name);
                            //   toastr.success(`Valid Code !`);
                              if(result != ''){
                                $('#txtAddDeviceName').val(result[0].product_name);
                                toastr.success(`Valid Code !`);
                              }else{
                                toastr.error(`Error: Invalid Code,PLEASE CALL THE ATTENTION OF IMMEDIATE SUPERVISOR`);
                                $('#txtAddDeviceName').val('');
                              }
                            }
                          });
                        // toastr.error(`Error: Invalid Code,PLEASE CALL THE ATTENTION OF IMMEDIATE SUPERVISOR`);
        ...
        $("#txtEditDeviceBarcode").keyup(function(e){
            let code = $(this).val();
            $.ajax({
                type: 'GET',
                url: `http://rapid/NAAYES/api/ypics_ts_ppts.php`,
                ...
                success: function (response) {
                    if(response.is_success === 'true'){
                        $('#txtEditDeviceName').val(response['NAME']);
                        toastr.success(`Valid Code !`);
                    }else{
                        // DITO KA MAGCCODE BUKAS DARREN GAGAWAN MO NG FUNCTION!!!
                          $.ajax({
                            ...
                            success: function (JsonObject) {
                                let result = JsonObject['device'];
                              if(result != ''){
                                $('#txtEditDeviceName').val(result[0].product_name);
                                toastr.success(`Valid Code !`);
                              }else{
                                toastr.error(`Error: Invalid Code,PLEASE CALL THE ATTENTION OF IMMEDIATE SUPERVISOR`);
